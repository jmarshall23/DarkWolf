...
void G_Script_ScriptLoad( void ) {
	...
	sys->Cvar_Register( &g_scriptDebug, "g_scriptDebug", "0", 0 );
	...
	sys->FS_Read( level.scriptEntity, len, f );
...
void G_Script_ScriptParse( gentity_t *ent ) {
	...
	buildScript = sys->Cvar_VariableIntegerValue( "com_buildScript" );
	buildScript = qtrue;
...
//----(SA)	added a bit more
						if (    buildScript && (
							...
							if ( strlen( token ) ) { // we know there's a [0], but don't know if it's '0'
								sys->SendServerCommand( ent->s.number, va( "addToBuild %s\n", token ) );
							}
						}
					}
//----(SA)	end
...
//	VectorCopy( ent->r.currentOrigin, ent->pos1 );
	...
	if ( ent->spawnflags & 1 ) {
		...
		sys->UnlinkEntity( ent ); // make sure it's not visible
		return;
	}
