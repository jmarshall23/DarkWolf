...
/*
	...
	if ( sys->InPVS (player->r.currentOrigin, ent->s.origin) )
	{
...
//----(SA)	end
	...
	sys->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin,
...
int G_PredictMissile( gentity_t *ent, int duration, vec3_t endPos, qboolean allowBounce ) {
	...
	for ( time = level.time + FRAMETIME; time < level.time + duration; time += FRAMETIME ) {
		...
		// trace a line from the previous position to the current position,
		// ignoring interactions with the missile owner
		sys->Trace( &tr, org, ent->r.mins, ent->r.maxs, origin,
					ent->r.ownerNum, ent->clipmask );
...
void G_RunSpit( gentity_t *ent ) {
	...
	if ( rand() % 100 > 60 ) {
		...
		sys->Trace( &tr, ent->r.currentOrigin, NULL, NULL, end,
...
// jpw
	case WP_DYNAMITE:
		...
		sys->SendServerCommand( self - g_entities, va( "dp %d", ( bolt->nextthink - level.time ) / 1000 ) );
// JPW NERVE
		if ( g_gametype.integer != GT_SINGLE_PLAYER ) {
// check if player is in trigger objective field -- swiped from G_TouchTriggers()
			...
			for ( i = 0 ; i < num ; i++ ) {
				...
				if ( !strcmp( hit->classname,"trigger_objective_info" ) ) {
					if ( hit->track ) {
						sys->SendServerCommand( -1, va( "cp \"%s\"", va( "Det charge planted near %s!", hit->track ) ) );
					} else {
						sys->SendServerCommand( -1, va( "cp \"%s\"", va( "Det charge planted near objective %d!", hit->count ) ) );
					}
					i = num;
				}
			}
...
void fire_lead( gentity_t *self, vec3_t start, vec3_t dir, int damage ) {
	...
	sys->Trace( &tr, start, NULL, NULL, end, self->s.number, MASK_SHOT );
	if ( tr.surfaceFlags & SURF_NOIMPACT ) {
		return;
	}
...
//	vec3_t		spot2;
	...
	sys->Trace( &tr, self->r.currentOrigin, NULL, NULL, other->r.currentOrigin, self->s.number, MASK_SHOT );
