...
void DropToFloorG( gentity_t *ent ) {
	...
	sys->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, dest, ent->s.number, MASK_SOLID );
...
void moveit( gentity_t *ent, float yaw, float dist ) {
	...
	sys->Trace( &tr, ent->r.currentOrigin, mins, maxs, origin, ent->s.number, MASK_SHOT );
...
//		AICast_AudibleEvent( self->s.number, self->r.currentOrigin, 384 );
...

		{
			...
			sys->Trace( &trace, self->r.currentOrigin, self->r.mins, self->r.maxs, end, self->s.number, MASK_SHOT );
...
void Props_Chair_Think( gentity_t *self ) {
	...
	} else
	{
		...
		sys->Trace( &tr, self->r.currentOrigin, mins, maxs, self->s.pos.trBase, self->s.number, MASK_SHOT );
...
qboolean Prop_Touch( gentity_t *self, gentity_t *other, vec3_t v ) {
	...
	sys->Trace( &tr, start, mins, maxs, end, self->s.number, MASK_SHOT );
...
//	sys->Trace( &tr, start, mins, maxs, end, self->s.number, MASK_SHOT );
	sys->Trace( &tr, start, mins, maxs, end, self->s.number, MASK_MISSILESHOT );
...
qboolean validOilSlickSpawnPoint( vec3_t point, gentity_t *ent ) {
	...
	sys->Trace( &tr, point, NULL, NULL, end, ent->s.number, MASK_SHOT );
...
//----(SA)	modified
	...
	sys->SetConfigstring( CS_SKYBOXORG, va( "%.2f %.2f %.2f %.1f %i %.2f %.2f %.2f %i %i", ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], fov_x, (int)isfog, fogv[0], fogv[1], fogv[2], fogn, fogf ) );
//----(SA)	end
}
...
void props_statue_blocked( gentity_t *ent ) {
	...
	sys->Trace( &trace, start, mins, maxs, end, ent->s.number, MASK_SHOT );
...
void props_locker_mass( gentity_t *ent ) {
	...
	sys->SetConfigstring( CS_TARGETEFFECT, ent->dl_shader );    //----(SA)	allow shader to be set from entity
