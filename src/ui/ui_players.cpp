...
tryagain:
	...
	if ( item->classname ) {
		pi->weaponModel = sys->R_RegisterModel( item->world_model[0] );
	}
...
//		weaponNum = WP_MACHINEGUN;	//----(SA)	removing old weapon references
	...
	pi->flashModel = sys->R_RegisterModel( path );
...
static void UI_PositionEntityOnTag( refEntity_t *entity, const refEntity_t *parent,
	...
	// lerp the tag
	sys->CM_LerpTag( &lerped, (const refEntity_t *)parent, (const char *)tagName, 0 );
...
static void UI_PositionRotatedEntityOnTag( refEntity_t *entity, const refEntity_t *parent,
	...
	// lerp the tag
	sys->CM_LerpTag( &lerped, parent, tagName, 0 );
...
static void UI_SetLerpFrameAnimation( playerInfo_t *ci, lerpFrame_t *lf, int newAnimation ) {
	...
	if ( newAnimation < 0 || newAnimation >= MAX_ANIMATIONS ) {
		sys->Error( va("Bad animation number (UI_SLFA): %i", newAnimation) );
	}
...
//	refEntity_t		barrel;
	...
	if ( pi->pendingWeapon != -1 && dp_realtime > pi->weaponTimer ) {
		...
		if ( pi->currentWeapon != pi->weapon ) {
			sys->S_StartLocalSound( sys->S_RegisterSound( "sound/weapons/change.wav" ), CHAN_LOCAL );
		}
	}
...
//		 &torso.oldframe, &torso.frame, &torso.backlerp );
	...
	sys->R_AddRefEntityToScene( &torso );
	...
	sys->R_AddRefEntityToScene( &head );
	...
	if ( dp_realtime <= pi->muzzleFlashTime ) {
		...
		if ( pi->flashDlightColor[0] || pi->flashDlightColor[1] || pi->flashDlightColor[2] ) {
			sys->R_AddLightToScene( flash.origin, 200 + ( rand() & 31 ), pi->flashDlightColor[0],
	...
	if ( pi->backpackModel ) {
		...
		sys->R_AddRefEntityToScene( &backpack );
	}
	...
	if ( pi->helmetModel ) {
		...
		sys->R_AddRefEntityToScene( &helmet );
	}
	...
	//
	if ( pi->chat ) {
		UI_PlayerFloatSprite( pi, origin, sys->R_RegisterShaderNoMip( "sprites/balloon3" ) );
	}
...
//	origin[2] += 100;	// + = above, - = below
	sys->R_AddLightToScene( origin, 500, 1.0, 1.0, 1.0, 0 );

	origin[0] -= 100;
	origin[1] -= 100;
	origin[2] -= 100;
	sys->R_AddLightToScene( origin, 500, 1.0, 0.0, 0.0, 0 );
...
//	Com_sprintf( filename, sizeof( filename ), "models/players/%s/lower_%s.skin", modelName, skinName );
	...
	pi->legsSkin = sys->R_RegisterSkin( filename );

//	Com_sprintf( filename, sizeof( filename ), "models/players/%s/upper_%s.skin", modelName, skinName );
	Com_sprintf( filename, sizeof( filename ), "models/players/%s/body_%s.skin", modelName, skinName );  // NERVE - SMF - make this work with wolf
	pi->torsoSkin = sys->R_RegisterSkin( filename );

	Com_sprintf( filename, sizeof( filename ), "models/players/%s/head_%s.skin", modelName, skinName );
	pi->headSkin = sys->R_RegisterSkin( filename );
...
qboolean UI_RegisterClientModelname( playerInfo_t *pi, const char *modelSkinName ) {
	...
	// NERVE - SMF - determine skin
	{
		...
		teamval = sys->Cvar_VariableValue( "mp_team" );
		...
		var = sys->Cvar_VariableValue( "mp_playerType" );
...
//	Com_sprintf( filename, sizeof( filename ), "models/players/%s/lower.md3", modelName );
	...
	pi->legsModel = sys->R_RegisterModel( filename );
	if ( !pi->legsModel ) {
...
//	Com_sprintf( filename, sizeof( filename ), "models/players/%s/upper.md3", modelName );
	Com_sprintf( filename, sizeof( filename ), "models/players/%s/body.mds", modelName ); // NERVE - SMF - make this work with wolf
	pi->torsoModel = sys->R_RegisterModel( filename );
	if ( !pi->torsoModel ) {
		Com_Printf( "Failed to load model file %s\n", filename );
		return qfalse;
	}
	...
	pi->headModel = sys->R_RegisterModel( filename );
	if ( !pi->headModel ) {
		Com_Printf( "Failed to load model file %s\n", filename );
		return qfalse;
	}
	...
	if ( backpack ) {
		pi->backpackModel = sys->R_RegisterModel( va( "models/players/%s/%s", modelName, backpack ) );
	}

	if ( helmet ) {
		pi->helmetModel = sys->R_RegisterModel( va( "models/players/%s/%s", modelName, helmet ) );
	}
