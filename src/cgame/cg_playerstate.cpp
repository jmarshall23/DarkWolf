...
//				break;
	...
	if ( !cg.lowAmmoWarning ) {
		...
		sys->S_StartLocalSound( cgs.media.noAmmoSound, CHAN_LOCAL_SOUND );
	}
...
//	sys->R_SetFog(FOG_CMD_SWITCHFOG, FOG_MAP,20,0,0,0,0);
	...
	sys->Cvar_Set( "cg_notebookpages", "3" );           // (SA) TEMP: clear notebook pages on spawn (cept for page 1&2)  this is temporary
	sys->Cvar_Set( "ui_notebookCurrentPage", "0" ); // (SA) TEMP: clear notebook pages on spawn (cept for page 1)  this is temporary
...
void CG_CheckLocalSounds( playerState_t *ps, playerState_t *ops ) {
	...
	if ( ps->persistant[PERS_REWARD_COUNT] > ops->persistant[PERS_REWARD_COUNT] ) {
		...
		case REWARD_IMPRESSIVE:
			sys->S_StartLocalSound( cgs.media.impressiveSound, CHAN_ANNOUNCER );
