...
void Use_Target_Print( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	if ( activator->client && ( ent->spawnflags & 4 ) ) {
		sys->SendServerCommand( activator - g_entities, va( "cp \"%s\"", ent->message ) );
		return;
	}
	...
	sys->SendServerCommand( -1, va( "cp \"%s\"", ent->message ) );
}
...
void target_laser_think( gentity_t *self ) {
	...
	sys->Trace( &tr, self->s.origin, NULL, NULL, end, self->s.number, CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE );
...
void target_location_linkup( gentity_t *ent ) {
	...
	sys->SetConfigstring( CS_LOCATIONS, "unknown" );
	...
	for ( i = 0, ent = g_entities, n = 1;
		...
		if ( ent->classname && !Q_stricmp( ent->classname, "target_location" ) ) {
			...
			sys->SetConfigstring( CS_LOCATIONS + n, ent->message );
			n++;
...
//		r,g,b
//		time to complete
	sys->SetConfigstring( CS_FOGVARS, va( "%f %f %f %f %f %f %i", ent->accuracy, ent->random, 1.0f, (float)ent->dl_color[0], (float)ent->dl_color[1], (float)ent->dl_color[2], ent->s.time ) );
}
