...
static void G_LoadArenas( void ) {
	...
	if ( !f ) {
		sys->Printf( va( S_COLOR_RED "file not found: %s\n", filename ) );
		return;
	}
	if ( len >= MAX_ARENAS_TEXT ) {
		sys->Printf( va( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_ARENAS_TEXT ) );
		sys->FS_FCloseFile( f );
		return;
	}
...
static void PlayerIntroSound( const char *modelAndSkin ) {
	...
	sys->SendConsoleCommand( EXEC_APPEND, va( "play sound/player/announce/%s.wav\n", skin ) );
}
	...
	for ( n = 0; n < g_numBots ; n++ ) {
		...
		if ( i >= g_maxclients.integer ) {
			...
			if ( num <= 0 ) {
				...
				Q_CleanStr( netname );
				sys->SendConsoleCommand( EXEC_INSERT, va( "addbot %s %i %s %i\n", netname, skill, teamstr, 0 ) );
				return;
			}
		}
	}
...
void G_CheckBotSpawn( void ) {
	...
	for ( n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++ ) {
		...
		if ( g_gametype.integer == GT_SINGLE_PLAYER ) {
			sys->GetUserinfo( botSpawnQueue[n].clientNum, userinfo, sizeof( userinfo ) );
...
qboolean G_BotConnect( int clientNum, qboolean restart ) {
	...
	sys->GetUserinfo( clientNum, userinfo, sizeof( userinfo ) );
	...
	if ( !BotAISetupClient( clientNum, &settings ) ) {
		sys->DropClient( clientNum, "BotAISetupClient failed" );
		return qfalse;
	}
...
static void G_AddBot( const char *name, int skill, const char *team, int delay ) {
	...
	sys->SetUserinfo( clientNum, userinfo );
...
void Svcmd_AddBot_f( void ) {
	...
	if ( level.time - level.startTime > 1000 &&
		 sys->Cvar_VariableIntegerValue( "cl_running" ) ) {
		sys->SendServerCommand( -1, "loaddeferred\n" );   // spelling fixed (SA)
	}
}
...
static void G_SpawnBots( char *botList, int baseDelay ) {
	...
	while( *p ) {
		...
		// does "Bad Things"
		sys->SendConsoleCommand( EXEC_INSERT, va("addbot %s %i free %i\n", bot, skill, delay) );
...
static void G_LoadBots( void ) {
	...
	if ( len >= MAX_BOTS_TEXT ) {
		sys->Printf( va( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_BOTS_TEXT ) );
		sys->FS_FCloseFile( f );
		return;
	}
...
void G_InitBots( qboolean restart ) {
	...
	sys->Cvar_Register( &bot_minplayers, "bot_minplayers", "0", CVAR_SERVERINFO );
	...
	if( g_gametype.integer == GT_SINGLE_PLAYER ) {
		...
		if ( fragLimit ) {
			sys->Cvar_Set( "fraglimit", strValue );
		}
		else {
			sys->Cvar_Set( "fraglimit", "0" );
		}

		strValue = Info_ValueForKey( arenainfo, "timelimit" );
		timeLimit = atoi( strValue );
		if ( timeLimit ) {
			sys->Cvar_Set( "timelimit", strValue );
		}
		else {
			sys->Cvar_Set( "timelimit", "0" );
		}

		if ( !fragLimit && !timeLimit ) {
			sys->Cvar_Set( "fraglimit", "10" );
			sys->Cvar_Set( "timelimit", "0" );
		}
