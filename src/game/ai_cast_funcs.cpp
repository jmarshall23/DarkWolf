...
char* AIFunc_DoorMarkerStart(cast_state_t* cs, int doorEntNum, int markerNum);
...
float AICast_GetRandomViewAngle(cast_state_t* cs, float tracedist) {
	...
	while (cnt++ < 4)
	{
		...
		//
		sys->Trace(&trace, start, NULL, NULL, end, passent, contents_mask);
		//
		if (trace.fraction >= 1) {
			return vec[YAW];
		}
...
Shared helpers
  ...
  Small state-management utilities used by multiple AI thinking functions.
============
*/
static void AICast_ResetMoveStateForDanger(cast_state_t* cs) {
	cs->castScriptStatus.scriptGotoId = -1;
	cs->movestate = MS_DEFAULT;
	cs->movestateType = MSTYPE_NONE;
}

static void AICast_SetupDangerAvoidance(cast_state_t* cs) {
	cs->takeCoverTime = cs->dangerEntityValidTime + 1000;
	cs->attackcrouch_time = 0;
}

static qboolean AICast_PrepareDangerAvoidanceAlways(cast_state_t* cs) {
	if (cs->dangerEntityValidTime < level.time) {
		return qfalse;
	}

	AICast_GetTakeCoverPos(cs, cs->dangerEntity, cs->dangerEntityPos, cs->takeCoverPos);
	AICast_SetupDangerAvoidance(cs);
	return qtrue;
}

static qboolean AICast_PrepareDangerAvoidanceIfPossible(cast_state_t* cs) {
	if (cs->dangerEntityValidTime < level.time) {
		return qfalse;
	}
...
static qboolean AICast_PrepareDangerAvoidanceIfPossibleAndResetMove(cast_state_t* cs) {
	if (!AICast_PrepareDangerAvoidanceIfPossible(cs)) {
		return qfalse;
	}

	AICast_ResetMoveStateForDanger(cs);
	return qtrue;
}

static qboolean AICast_ShouldWaitForConnectingClient(int entnum, gentity_t* ent) {
	if (entnum >= MAX_CLIENTS) {
		return qfalse;
	}

	if (ent->client && ent->client->pers.connected == CON_CONNECTING) {
		return qtrue;
	}
...
static void AICast_CopyEntityOrigin(gentity_t* ent, vec3_t destorg) {
	if (ent->client) {
		VectorCopy(ent->client->ps.origin, destorg);
	}
	else {
		VectorCopy(ent->r.currentOrigin, destorg);
	}
}

static void AICast_SetHeadLookFlag(cast_state_t* cs) {
...
static void AICast_UpdateIdleViewAngles(cast_state_t* cs, float tracedist) {
	if (cs->nextIdleAngleChange < level.time) {
		if ((cs->nextIdleAngleChange + 3000) < level.time) {
			cs->idleYaw = AICast_GetRandomViewAngle(cs, tracedist);
...
bot_moveresult_t* AICast_MoveToPos(cast_state_t* cs, vec3_t pos, int entnum) {
	...
	if (aicast_debug.integer == 2 && (g_entities[cs->entityNum].aiName && !strcmp(aicast_debugname.string, g_entities[cs->entityNum].aiName))) {
		sys->AAS_RT_ShowRoute(cs->bs->origin, cs->bs->areanum, goal.areanum);
	}
	//
	...
	if (!(cs->aiFlags & AIFL_EXPLICIT_ROUTING) || (entnum < 0) || Q_strcasecmp(g_entities[entnum].classname, "ai_marker")) {
		// use AAS routing
		sys->BotMoveToGoal(&lmoveresult, bs->ms, &goal, tfl);
		...
		if (lmoveresult.failure) {
			...
			// clear all movement
			sys->EA_Move(cs->entityNum, vec3_origin, 0);
			...
			if (lmoveresult.flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_SWIMVIEW)) {
				...
				sys->EA_GetInput(bs->client, 0.1, &bi);
		...
		if ((dist = VectorNormalize(dir)) < 64) {
			sys->EA_Move(cs->entityNum, dir, 100.0 + 300.0 * (dist / 64.0));
		}
		else {
			sys->EA_Move(cs->entityNum, dir, 400);
		}
	...
	sys->EA_GetInput(cs->bs->client, (float) level.time / 1000, &bi);
	G_Printf("spd: %i\n", (int)bi.speed );
	}
	*/
...
char* AIFunc_Idle(cast_state_t* cs) {
	...
	if (AICast_PrepareDangerAvoidanceAlways(cs)) {
	...
	if (cs->lastEnemy >= 0) {
		...
		else if (cs->attributes[TACTICAL] || cs->idleYawChange) {
			AICast_UpdateIdleViewAngles(cs, 512);
		}
	}
...
char* AIFunc_IdleStart(cast_state_t* cs) {
	...
	sys->BotInitAvoidReach(cs->bs->ms);
...
char* AIFunc_InspectFriendly(cast_state_t* cs) {
	...
	// do we need to avoid a danger?
	if (AICast_PrepareDangerAvoidanceIfPossibleAndResetMove(cs)) {
		return AIFunc_AvoidDangerStart(cs);
	}
	//
	// are we waiting for a door?
	...
	if (!followent->inuse) {
		if (AICast_ShouldWaitForConnectingClient(cs->followEntity, followent)) {
	...
	AICast_CopyEntityOrigin(followent, destorg);
	...
	if (!(dist < cs->followDist && (ent->waterlevel || (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE)))) {
		...
		if (!moved &&
			...
			if (!simTest) {
				...
				sys->Trace(&tr, cs->bs->origin, NULL /*g_entities[cs->entityNum].r.mins*/, NULL /*g_entities[cs->entityNum].r.maxs*/, followent->r.currentOrigin, cs->entityNum, g_entities[cs->entityNum].clipmask);
			...
			if (simTest) {
				...
				//sys->EA_Move(cs->entityNum, dir, 400);
				sys->EA_GetInput(cs->entityNum, (float)level.time / 1000, &bi);
				...
				if (move.stopevent == PREDICTSTOP_HITENT) { // success!
					sys->EA_Move(cs->entityNum, dir, 400);  // set the movement
					vectoangles(dir, cs->ideal_viewangles);
					cs->ideal_viewangles[2] *= 0.5;
					moved = qtrue;
				}
				else {    // clear movement
					//sys->EA_Move(cs->entityNum, dir, 0);
				}
			}
		}
		//
		if (!moved) {
			...
			if (!moveresult || moveresult->failure) {
				...
				if (!(cs->aiFlags & AIFL_MISCFLAG2)) {
					if (sys->AAS_GetRouteFirstVisPos(followent->r.currentOrigin, cs->bs->origin, cs->travelflags, cs->takeCoverEnemyPos)) {
...
char* AIFunc_InspectBulletImpactStart(cast_state_t* cs) {
	...
	if (!sys->InPVS(cs->bulletImpactStart, cs->bs->origin)) {
		// if it fails, then just look at the source
		sys->AAS_GetRouteFirstVisPos(g_entities[cs->bulletImpactEntity].s.pos.trBase, cs->bs->origin, cs->travelflags, cs->bulletImpactStart);
	}
	//
...
char* AIFunc_InspectAudibleEvent(cast_state_t* cs) {
	...
	if (!(dist < cs->followDist && (ent->waterlevel || (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE)))) {
		...
		if (!moved &&
			...
			if (!simTest) {
				// trace will eliminate most unsuccessful paths
				sys->Trace(&tr, cs->bs->origin, NULL /*g_entities[cs->entityNum].r.mins*/, NULL /*g_entities[cs->entityNum].r.maxs*/, destorg, cs->entityNum, g_entities[cs->entityNum].clipmask);
				if (tr.fraction == 1) {
					simTest = qtrue;
				}
			}
		...
		if (!moved) {
			...
			if (moveresult && moveresult->failure) {
				...
				// if we can get a visible target, then face it
				if (sys->AAS_GetRouteFirstVisPos(cs->audibleEventOrg, cs->bs->origin, cs->travelflags, destorg)) {
					cs->aiFlags |= AIFL_MISCFLAG2;
				...
				if (sys->InPVS(destorg, cs->bs->origin)) {
		...
		else
		{
			if (cs->followEntity == cs->leaderNum) {
				if (dist < AICAST_LEADERDIST_MIN) {
					...
					sys->Trace(&tr, cs->bs->origin, cs->bs->cur_ps.mins, cs->bs->cur_ps.maxs, g_entities[cs->followEntity].r.currentOrigin, cs->entityNum, g_entities[cs->entityNum].clipmask);
...
char* AIFunc_BattleRoll(cast_state_t* cs) {
	...
	if (cs->thinkFuncChangeTime > level.time - 1000) {
		...
		sys->EA_Move(cs->entityNum, dir, 300);
...
char* AIFunc_BattleHunt(cast_state_t* cs) {
	...
	// if the entity is not ready yet
	if (!followent->inuse) {
		if (!AICast_ShouldWaitForConnectingClient(cs->enemyNum, followent)) {
			cs->enemyNum = -1;
		}
		return AIFunc_IdleStart(cs);
	}
		...
		if (AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
			...
			for (i = 0; i < numEnemies; i++) {
				...
				if ((AICast_EntityVisible(cs, enemies[i], qfalse) && AICast_CheckAttack(cs, enemies[i], qfalse)) ||
					...
					sys->EA_GetInput(cs->entityNum, (float)level.time / 1000, &bi_back);
					if (AICast_MoveToPos(cs, g_entities[enemies[i]].client->ps.origin, enemies[i])) {
						...
						sys->EA_ResetInput(cs->entityNum, &bi_back);
					}
				}
			}
		}
...
char* AIFunc_BattleChase(cast_state_t* cs) {
	...
	if (AICast_EntityVisible(cs, cs->enemyNum, qtrue) && VectorDistance(cs->bs->origin, destorg) < 384) {
		if (sys->PointContents(destorg, cs->enemyNum) & (CONTENTS_DONOTENTER | CONTENTS_DONOTENTER_LARGE)) {
	...
	*/
	...
	if (!moved && cs->leaderNum < 0 &&
		...
		// trace will eliminate most unsuccessful paths
		sys->Trace(&tr, cs->bs->origin, NULL, NULL, followent->r.currentOrigin, cs->entityNum, g_entities[cs->entityNum].clipmask);
		if (tr.entityNum == followent->s.number) {
			...
			if (move.stopevent == PREDICTSTOP_HITENT) { // success!
				...
				if ((VectorLength(followent->client->ps.velocity) < 50) && (dist < 10 + (sqrt(cs->bs->cur_ps.maxs[0] * cs->bs->cur_ps.maxs[0] * 8.0) / 2.0 + sqrt(followent->client->ps.maxs[0] * followent->client->ps.maxs[0] * 8.0) / 2.0))) {
					...
					sys->Trace(&tr, cs->bs->origin, cs->bs->cur_ps.mins, cs->bs->cur_ps.maxs, followent->r.currentOrigin, cs->entityNum, g_entities[cs->entityNum].clipmask);
					if (tr.entityNum != followent->s.number) {
						if (level.time % 6000 < 2000) {
							sys->EA_MoveRight(cs->entityNum);
						}
						else {
							sys->EA_MoveLeft(cs->entityNum);
						}
					}
				}
				vectoangles(dir, cs->ideal_viewangles);
		...
		if (cs->leaderNum >= 0) {
			if (cs->combatGoalTime < level.time) {
				if (cs->attackSpotTime < level.time) {
					...
					if (sys->AAS_FindAttackSpotWithinRange(cs->entityNum, cs->leaderNum, cs->enemyNum, MAX_LEADER_DIST, AICAST_TFL_DEFAULT, cs->combatGoalOrigin)) {
			...
			if (cs->combatGoalTime < level.time) {
				if (cs->attackSpotTime < level.time) {
					cs->attackSpotTime = level.time + 500 + rand() % 500;
					if (sys->AAS_FindAttackSpotWithinRange(cs->entityNum, cs->entityNum, cs->enemyNum, 512, AICAST_TFL_DEFAULT, cs->combatGoalOrigin)) {
						cs->combatGoalTime = level.time + 2000;
					}
				}
			}
...
#define BATTLE_CHASE_ACCEL_TIME     300
	...
	// if we are going to reach them soon, predict the attack
	{
		...
		// do we went to play a diving animation into a cover position?
			...
			(AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, move.endpos, (qboolean)(cs->attackcrouch_time > level.time), qfalse))) {
			cs->takeCoverTime = 0;
		...
		//
			...
			starttravel = sys->AAS_AreaTravelTimeToGoalArea(cs->bs->areanum, cs->bs->origin, destarea, cs->travelflags);
			simtravel = sys->AAS_AreaTravelTimeToGoalArea(simarea, move.endpos, destarea, cs->travelflags);
		...
		if ((danger->s.weapon == WP_GRENADE_LAUNCHER || danger->s.weapon == WP_GRENADE_PINEAPPLE) &&
			...
			if (!AICast_SameTeam(cs, danger->r.ownerNum)) {
				if (G_PredictMissile(danger, danger->nextthink - level.time, cs->takeCoverPos, qfalse)) {
					...
					sys->Trace(&tr, cs->takeCoverPos, ent->r.mins, ent->r.maxs, end, cs->entityNum, MASK_SOLID);
					...
					if (!tr.startsolid && (tr.fraction < 1.0) &&
						...
						sys->Trace(&tr, cs->bs->origin, ent->r.mins, ent->r.maxs, cs->takeCoverPos, cs->entityNum, MASK_SOLID);
	...
	if (cs->dangerEntity >= MAX_CLIENTS && Distance(cs->dangerEntityPos, cs->bs->origin) > cs->dangerDist * 1.5) {
		...
		if (dist > 8) {
			...
			if (ent->s.onFireEnd < level.time && cs->enemyNum >= 0 && cs->vislist[cs->enemyNum].real_visible_timestamp && (cs->vislist[cs->enemyNum].real_visible_timestamp > level.time - 10000)) {
				AICast_UpdateVisibility(&g_entities[cs->entityNum], &g_entities[cs->enemyNum], qfalse, (qboolean)(cs->vislist[cs->enemyNum].real_visible_timestamp == cs->vislist[cs->enemyNum].lastcheck_timestamp));
			}
...
// DISABLED: doesn't work well with new AI system
	...
	if (VectorLength(cs->takeCoverPos) > 1 && dist > 8) {
		...
		if (!(cs->aiFlags & AIFL_MISCFLAG1) && !AICast_VisibleFromPos(cs->vislist[cs->enemyNum].real_visible_pos, cs->enemyNum, move.endpos, cs->entityNum, qfalse)
			...
			&& sys->AAS_PointAreaNum(move.endpos)) { // make sure the endpos is in a valid area
...
//G_Printf("aborting, enemy is attackable\n");
	...
	if (cs->vislist[cs->enemyNum].lastcheck_timestamp > cs->vislist[cs->enemyNum].real_visible_timestamp ||
		...
		if ((cs->leaderNum < 0) &&
			...
			// trace will eliminate most unsuccessful paths
			sys->Trace(&tr, cs->bs->origin, g_entities[cs->entityNum].r.mins, g_entities[cs->entityNum].r.maxs, followent->r.currentOrigin, cs->entityNum, g_entities[cs->entityNum].clipmask);
			if (tr.entityNum == followent->s.number) {
				// try walking straight to them
				VectorSubtract(followent->r.currentOrigin, cs->bs->origin, dir);
				VectorNormalize(dir);
...
//		if ((cs->grenadeGrabFlag == -1) || (cs->grenadeGrabFlag == qtrue && level.time > danger->nextthink - 2000)) {	// kick
		...
		/*
					...
					if (cs->grenadeGrabFlag == qtrue) {
						...
						VectorScale( dir, speed, danger->s.pos.trDelta );
						sys->LinkEntity( danger );
					} else if (ent->client->ps.legsTimer < 800) {
						// stop showing the grenade
						sys->UnlinkEntity( danger );
					}
				}
		*/
		//
...
char* AIFunc_Battle(cast_state_t* cs) {
	...
	if ((cs->bs->cur_ps.weaponTime < 100)   // if reloading, don't chase until ready
		...
		if (!cs->bs->areanum) {
			if (cs->obstructingTime >= level.time) {
				// move there
				sys->EA_Move(cs->entityNum, cs->takeCoverPos, 200);
			}
	...
	*/
	...
	if ((cs->attributes[AGGRESSION] < 1.0) &&
		...
		if (DotProduct(aim, enemyVec) > 0.97) {
			...
			sys->EA_GetInput(cs->entityNum, (float)level.time / 1000, &bi_back);
			sys->EA_ResetInput(cs->entityNum, NULL);
			...
			if (move.groundEntityNum == ENTITYNUM_WORLD &&
				...
				// good enough
				if (AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, move.endpos, (qboolean)(cs->bs->cur_ps.viewheight == cs->bs->cur_ps.crouchViewHeight), qfalse)) {
					cs->takeCoverTime = 0;
