...
void DeathmatchScoreboardMessage( gentity_t *ent ) {
	...
	sys->SendServerCommand( ent - g_entities, va( "scores %i %i %i%s", i,
...
int ClientNumberFromString( gentity_t *to, char *s ) {
	...
	sys->SendServerCommand( to - g_entities, va( "print \"User %s is not on the server\n\"", s ) );
	return -1;
}
...
void G_setfog( char *fogstring ) {
	sys->SetConfigstring( CS_FOGVARS, fogstring );
}
...
void Cmd_LevelShot_f( gentity_t *ent ) {
	...
	if ( g_gametype.integer != 0 ) {
		sys->SendServerCommand( ent - g_entities,
								"print \"Must be in g_gametype 0 for levelshot\n\"" );
		return;
	}

	BeginIntermission();
	sys->SendServerCommand( ent - g_entities, "clientLevelShot" );
}
...
void Cmd_Team_f( gentity_t *ent ) {
	...
	if ( g_gametype.integer == GT_WOLF ) {
		sys->Argv( 2, ptype, sizeof( ptype ) );
		sys->Argv( 3, weap, sizeof( weap ) );
		sys->Argv( 4, pistol, sizeof( pistol ) );
		sys->Argv( 5, grenade, sizeof( grenade ) );
		sys->Argv( 6, skinnum, sizeof( skinnum ) );
...
void G_SayTo( gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message ) { // removed static so it would link
	...
	// NERVE - SMF
	if ( mode == SAY_LIMBO ) {
		sys->SendServerCommand( other - g_entities, va( "%s \"%s%c%c%s\"",
...
void Cmd_Where_f( gentity_t *ent ) {
	sys->SendServerCommand( ent - g_entities, va( "print \"%s\n\"", vtos( ent->s.origin ) ) );
}
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	if ( !g_allowVote.integer ) {
		sys->SendServerCommand( ent - g_entities, "print \"Voting not allowed here.\n\"" );
		return;
	}
	...
	sys->Argv( 1, arg1, sizeof( arg1 ) );
	sys->Argv( 2, arg2, sizeof( arg2 ) );
	...
	sys->SetConfigstring( CS_VOTE_STRING, level.voteString );
...
void Cmd_Vote_f( gentity_t *ent ) {
	...
	sys->SendServerCommand( ent - g_entities, "print \"Vote cast.\n\"" );

	ent->client->ps.eFlags |= EF_VOTED;

	sys->Argv( 1, msg, sizeof( msg ) );
...
void Cmd_SetViewpos_f( gentity_t *ent ) {
	...
	sys->Argv( 4, buffer, sizeof( buffer ) );
...
//	start[2] += 24;
	...
	sys->Trace( &trace, start, mins, maxs, end, ent->s.number, MASK_SOLID | MASK_MISSILESHOT );
...
//		ent->s.eFlags &= ~EF_MELEE_ACTIVE;
	...
	return (qboolean) ( isthrown || force );
}
...
void Cmd_Activate_f( gentity_t *ent ) {
	...
	sys->Trace( &tr, offset, NULL, NULL, end, ent->s.number, ( CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE | CONTENTS_TRIGGER ) );
...
void Cmd_ClientMonsterSlickAngle (gentity_t *clent) {
	...
	sys->Argv( 2, s, sizeof( s ) );
	angle = atoi(s);

	// sanity check (also protect from cheaters)
	if (g_gametype.integer != GT_SINGLE_PLAYER && entnum != clent->s.number) {
		sys->DropClient( clent->s.number, "Dropped due to illegal ClientMonsterSlick command\n" );
		return;
	}
...
void ClientDamage( gentity_t *clent, int entnum, int enemynum, int id ) {
	...
	// sanity check (also protect from cheaters)
//	if (g_gametype.integer != GT_SINGLE_PLAYER && entnum != clent->s.number) {
//		sys->DropClient( clent->s.number, "Dropped due to illegal ClientDamage command\n" );
//		return;
//	}
	// -NERVE - SMF
