...
//		duration = MARK_TOTAL_TIME;
	...
	numFragments = sys->CM_MarkFragments( (int)orientation, (const vec3_t *)originalPoints,
	...
	for ( i = 0, mf = markFragments ; i < numFragments ; i++, mf++ ) {
		...
		if ( temporary ) {
			sys->R_AddPolyToScene( markShader, mf->numPoints, verts );
			continue;
		}
...
void CG_AddMarks( void ) {
	...
	for ( ; mp != &cg_activeMarkPolys ; mp = next ) {
		...
		sys->R_AddPolyToScene( mp->markShader, mp->poly.numVerts, mp->verts );
	}
}
