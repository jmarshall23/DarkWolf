...
int CG_SoundScriptPrecache( const char *name ) {
	...
	while ( sound ) {
		if ( !Q_strcasecmp( s, sound->name ) ) {
			...
			if ( !sound->streaming ) {
				while ( scriptSound ) {
					scriptSound->sfxHandle = sys->S_RegisterSound( scriptSound->filename );
	...
	if ( oldestSound ) {
		...
		if ( !sound->streaming ) {
			if ( !oldestSound->sfxHandle ) {
				oldestSound->sfxHandle = sys->S_RegisterSound( oldestSound->filename );
			}
			if ( sound->attenuation ) {
				sys->S_StartSound( org, entnum, sound->channel, oldestSound->sfxHandle );
			} else {
				sys->S_StartLocalSound( oldestSound->sfxHandle, sound->channel );
			}
		} else {
			sys->S_StartStreamingSound( oldestSound->filename, sound->looping ? oldestSound->filename : NULL, entnum, sound->channel, sound->attenuation );
		}
