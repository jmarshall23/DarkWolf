...
void CG_CheckOrderPending() {
	...
	if ( cgs.orderPending ) {
		...
		if ( cg_currentSelectedPlayer.integer == numSortedTeamPlayers ) {
			// to everyone
			sys->SendConsoleCommand( va( "cmd vsay_team %s\n", p2 ) );
		} else {
			// for the player self
			if ( sortedTeamPlayers[cg_currentSelectedPlayer.integer] == cg.snap->ps.clientNum && p1 ) {
				sys->SendConsoleCommand( va( "teamtask %i\n", cgs.currentOrder ) );
				//sys->SendConsoleCommand(va("cmd say_team %s\n", p2));
				sys->SendConsoleCommand( va( "cmd vsay_team %s\n", p1 ) );
			} else if ( p2 ) {
				//sys->SendConsoleCommand(va("cmd say_team %s, %s\n", ci->name,p));
				sys->SendConsoleCommand( va( "cmd vtell %d %s\n", sortedTeamPlayers[cg_currentSelectedPlayer.integer], p2 ) );
			}
		}
		if ( b ) {
			sys->SendConsoleCommand( b );
		}
...
static void CG_SetSelectedPlayerName() {
	if ( cg_currentSelectedPlayer.integer >= 0 && cg_currentSelectedPlayer.integer < numSortedTeamPlayers ) {
		clientInfo_t *ci = cgs.clientinfo + sortedTeamPlayers[cg_currentSelectedPlayer.integer];
		if ( ci ) {
			sys->Cvar_Set( "cg_selectedPlayerName", ci->name );
			sys->Cvar_Set( "cg_selectedPlayer", va( "%d", sortedTeamPlayers[cg_currentSelectedPlayer.integer] ) );
	...
	if ( ci ) {
		...
		for ( j = 0; j < PW_NUM_POWERUPS; j++ ) {
			if ( ci->powerups & ( 1 << j ) ) {
				...
				if ( item ) {
					CG_DrawPic( x, y, rect->w, rect->h, sys->R_RegisterShader( item->icon ) );
					x += 3;
					y += 3;
					return;
				}
...
static void CG_DrawAreaPowerUp( rectDef_t *rect, int align, float spacing, int font, float scale, vec4_t color ) {
	...
	for ( i = 0 ; i < active ; i++ ) {
		...
		if ( item ) {
			...
			CG_DrawPic( r2.x, r2.y, r2.w * .75, r2.h, sys->R_RegisterShader( item->icon ) );
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM] ) {
			...
			for ( j = 0; j <= PW_NUM_POWERUPS; j++ ) {
				if ( ci->powerups & ( 1 << j ) ) {
					...
					if ( item ) {
						CG_DrawPic( xx, y, PIC_WIDTH, PIC_WIDTH, sys->R_RegisterShader( item->icon ) );
						xx += PIC_WIDTH;
					}
				}
			}
...
void CG_KeyEvent( int key, qboolean down ) {
	...
	//if (key == sys->Key_GetKey("teamMenu") || !Display_CaptureItem(cgs.cursorX, cgs.cursorY)) {
