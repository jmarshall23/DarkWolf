...
void BotAI_Trace( bsp_trace_t *bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int passent, int contentmask ) {
	...
	sys->Trace( &trace, start, mins, maxs, end, passent, contentmask );
...
void QDECL BotAI_BotInitialChat( bot_state_t *bs, char *type, ... ) {
	...
	sys->BotInitialChat( bs->cs, type, mcontext, vars[0], vars[1], vars[2], vars[3], vars[4], vars[5], vars[6], vars[7] );
}
...
void BotInterbreeding( void ) {
	...
	if ( sys->GeneticParentsAndChildSelection( MAX_CLIENTS, ranks, &parent1, &parent2, &child ) ) {
...
void BotEntityInfo( int entnum, aas_entityinfo_t *info ) {
	sys->AAS_EntityInfo( entnum, info );
}
...
void BotAIRegularUpdate( void ) {
	if ( regularupdate_time < sys->AAS_Time() ) {
		sys->BotUpdateEntityItems();
		regularupdate_time = sys->AAS_Time() + 1;
	}
}
...
int BotAISetupClient( int client, struct bot_settings_s *settings ) {
	...
	bs->walker = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_WALKER, 0, 1 );
	numbots++;

	if ( sys->Cvar_VariableIntegerValue( "bot_testichat" ) ) {
...
int BotAIShutdownClient( int client ) {
	...
	sys->BotFreeCharacter( bs->character );
	//
...
int BotAILoadMap( int restart ) {
	...
	if ( !restart ) {
		...
		sys->BotLibLoadMap( mapname.string );
	}
...
int BotAIStartFrame( int time ) {
	...
	sys->Cvar_Update( &bot_rocketjump );
	sys->Cvar_Update( &bot_grapple );
	sys->Cvar_Update( &bot_fastchat );
	sys->Cvar_Update( &bot_nochat );
	sys->Cvar_Update( &bot_testrchat );
	sys->Cvar_Update( &bot_thinktime );
	// Ridah, set the default AAS world
	sys->AAS_SetCurrentWorld( 0 );
	sys->Cvar_Update( &memorydump );

	if ( memorydump.integer ) {
		sys->BotLibVarSet( "memorydump", "1" );
		sys->Cvar_Set( "memorydump", "0" );
	}
...
//----(SA)	didn't want to comment in as I wasn't sure of any implications of changing the aas_entityinfo_t and bot_entitystate_t structures.
			...
			*/
			//
			sys->BotLibUpdateEntity( i, &state );
		}
	...
	for ( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		sys->BotUserCommand( botstates[i]->client, &botstates[i]->lastucmd );
	}
...
int BotAISetup( int restart ) {
	...
	sys->Cvar_Register( &memorydump, "memorydump", "0", 0 );
