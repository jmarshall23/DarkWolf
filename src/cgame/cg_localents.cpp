...
void CG_FreeLocalEntity( localEntity_t *le ) {
	...
	// Ridah, debugging
	localEntCount--;
//	sys->Print( va("FreeLocalEntity: locelEntCount = %d\n", localEntCount) );
	// done.
...
localEntity_t   *CG_AllocLocalEntity( void ) {
	...
	// Ridah, debugging
	localEntCount++;
//	sys->Print( va("AllocLocalEntity: locelEntCount = %d\n", localEntCount) );
	// done.
...
//----(SA) end
		...
		sys->S_StartSound( trace->endpos, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.boneBounceSound );
...
//----(SA)	end
	...
	if ( hasFlame ) {
		...
		if ( VectorLength( flameDir ) == 0 ) {
			...
			sys->S_AddLoopingSound( 0, newOrigin, vec3_origin, cgs.media.flameBlowSound, (int)( 0.3 * 255.0 * flameAlpha ) );
		}
	}
	...
	// and floating levels
	if ( sys->CM_PointContents( trace.endpos, 0 ) & CONTENTS_NODROP ) {
...
//		if ( sys->CM_PointContents( le->refEntity.origin, 0 ) & CONTENTS_NODROP ) {
//			CG_FreeLocalEntity( le );
//			return;
//		}
...
void CG_AddClientCritter( localEntity_t *le ) {
	...
	while ( time <= cg.time ) {
		...
		if ( trace.fraction < 1.0 ) {
			...
			if ( VectorDistance( le->refEntity.origin, enemyPos ) > VectorDistance( le->refEntity.origin, newOrigin ) ) {
				...
				sys->CM_BoxTrace( &trace, le->refEntity.origin, newOrigin, NULL, NULL, 0, MASK_SOLID );
	...
	if ( le->loopingSound ) {
		if ( cg.time > le->refEntity.fadeStartTime ) {
			sys->S_AddLoopingSound( 0, le->refEntity.origin, vec3_origin, le->loopingSound, 255 - (int)( 255.0 * (float)( cg.time - le->refEntity.fadeStartTime ) / (float)( le->refEntity.fadeEndTime - le->refEntity.fadeStartTime ) ) );
		} else {
