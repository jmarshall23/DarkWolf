...
//	player->client->ps.serverCursorHint = HINT_EXIT;
	...
	sys->Trace( tr, offset, NULL, NULL, end, ps->clientNum, trace_contents );
...
void G_RemapTeamShaders() {
	...
	sys->SetConfigstring( CS_SHADERSTATE, BuildShaderStateConfig() );
#endif
}
...
void G_RegisterCvars( void ) {
	...
	if ( g_gameskill.integer < GSKILL_EASY || g_gameskill.integer > GSKILL_MAX ) {
		...
		sys->Cvar_Set( "g_gameskill", va( "%d", GSKILL_MEDIUM ) ); // default to medium
	}
	...
	for ( i = 0, cv = gameCvarTable ; i < gameCvarTableSize ; i++, cv++ ) {
		if ( cv->vmCvar ) {
			...
			if ( cv->modificationCount != cv->vmCvar->modificationCount ) {
				...
				if ( cv->trackChange ) {
					sys->SendServerCommand( -1, va( "print \"Server: %s changed to %s\n\"",
				...
				if ( !Q_stricmp( cv->cvarName, "g_playerStart" ) ) {
					...
					if ( player && cv->vmCvar->integer ) {
						...
						sys->Cvar_Set( "cg_norender", "0" );  // camera has started, render 'on'
						sys->Cvar_Set( "g_playerstart", "0" ); // reset calling of "playerstart" from script
					}
				}
			}
		}
...
//	sys->Cvar_Set( "g_missionStats", cmd );
	// changing to a configstring (should help w/ savegame, no?)
	sys->SetConfigstring( CS_MISSIONSTATS, cmd );
...
//	level.snd_fry = G_SoundIndex("sound/player/fry.wav");	// FIXME standing in lava / slime
	...
	if ( g_gametype.integer != GT_SINGLE_PLAYER && g_log.string[0] ) {
		...
		if ( !level.logFile ) {
			...
			sys->GetServerinfo( serverinfo, sizeof( serverinfo ) );
	...
	if ( g_gametype.integer == GT_SINGLE_PLAYER || sys->Cvar_VariableIntegerValue( "com_buildScript" ) ) {
...
void G_ShutdownGame( int restart ) {
	...
	if ( level.logFile ) {
		...
		G_LogPrintf( "------------------------------------------------------------\n" );
		sys->FS_FCloseFile( level.logFile );
	}
...
void ExitLevel( void ) {
	...
	if ( g_gametype.integer == GT_TOURNAMENT ) {
		if ( !level.restarted ) {
			RemoveTournamentLoser();
			sys->SendConsoleCommand( EXEC_APPEND, "map_restart 0\n" );
	...
	sys->SendConsoleCommand( EXEC_APPEND, "vstr nextmap\n" );
	level.changemap = NULL;
	level.intermissiontime = 0;
...
void QDECL G_LogPrintf( const char *fmt, ... ) {
	...
	sys->FS_Write( string, strlen( string ), level.logFile );
}
...
void LogExit( const char *string ) {
	...
	sys->SetConfigstring( CS_INTERMISSION, "1" );
...
void CheckExitRules( void ) {
	...
	if ( g_gametype.integer != GT_CTF && g_fraglimit.integer ) {
		if ( level.teamScores[TEAM_RED] >= g_fraglimit.integer ) {
			sys->SendServerCommand( -1, "print \"Red hit the fraglimit.\n\"" );
			LogExit( "Fraglimit hit." );
			return;
		}

		if ( level.teamScores[TEAM_BLUE] >= g_fraglimit.integer ) {
			sys->SendServerCommand( -1, "print \"Blue hit the fraglimit.\n\"" );
			LogExit( "Fraglimit hit." );
			return;
		}
		...
		for ( i = 0 ; i < g_maxclients.integer ; i++ ) {
			...
			if ( cl->ps.persistant[PERS_SCORE] >= g_fraglimit.integer ) {
				LogExit( "Fraglimit hit." );
				sys->SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " hit the fraglimit.\n\"",
												cl->pers.netname ) );
				return;
			}
		}
	...
	if ( g_gametype.integer == GT_CTF && g_capturelimit.integer ) {
		...
		if ( level.teamScores[TEAM_RED] >= g_capturelimit.integer ) {
			sys->SendServerCommand( -1, "print \"Red hit the capturelimit.\n\"" );
			LogExit( "Capturelimit hit." );
			return;
		}

		if ( level.teamScores[TEAM_BLUE] >= g_capturelimit.integer ) {
			sys->SendServerCommand( -1, "print \"Blue hit the capturelimit.\n\"" );
			LogExit( "Capturelimit hit." );
			return;
		}
	}
		...
		if ( level.reloadDelayTime ) {
			if ( level.reloadDelayTime < level.time ) {
				...
				if ( g_reloading.integer == RELOAD_NEXTMAP_WAITING ) {
					...
					sys->Cvar_Set( "savegame_loading", "2" ); // 2 means it's a restart, so stop rendering until we are loaded
					sys->SendConsoleCommand( EXEC_INSERT, "map_restart\n" );
				}
