...
//				f->blueLife = (int)(0.3*(1.0/speedScale)*(float)FLAME_BLUE_LIFE);
		...
		//if (firing) {
		//	sys->S_StartSound( org, cent->currentState.number, CHAN_AUTO, cgs.media.flameStartSound );
		//}
...
void CG_AddFlameSpriteToScene( flameChunk_t *f, float lifeFrac, float alpha ) {
	...
	sys->R_AddPolyToScene( flameShaders[frameNum], 4, verts );
	...
	while ( f ) {
		...
		for ( trav = f; trav; trav = trav->nextFlameChunk ) {
			...
			if ( trav->blueLife + 100 < ( cg.time - trav->timeStart ) ) {
				...
				sys->S_AddLoopingSound( trav->ownerCent, trav->org, vec3_origin, cgs.media.flameSound, (int)( 255.0 * ( 0.2 * ( trav->size / FLAME_MAX_SIZE ) ) ) );
