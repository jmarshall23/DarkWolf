...
void AICast_ScriptLoad( void ) {
	...
	sys->FS_Read( level.scriptAI, len, f );
...
void AICast_ScriptParse( cast_state_t *cs ) {
	...
	buildScript = (qboolean)sys->Cvar_VariableIntegerValue( "com_buildScript" );
	buildScript = qtrue;
...
//----(SA)	added a bit more
						if (    buildScript && (
							...
							if ( strlen( token ) ) { // we know there's a [0], but don't know if it's '0'
								sys->SendServerCommand( cs->entityNum, va( "addToBuild %s\n", token ) );
							}
						}
