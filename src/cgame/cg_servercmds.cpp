...
void CG_ParseServerinfo( void ) {
	...
	sys->Cvar_Set( "g_gametype", va( "%i", cgs.gametype ) );
...
// prolly should parse all CS_SERVERINFO keys automagically, but I don't want to break anything that might be improperly set for wolf SP, so I'm just parsing MP relevant stuff here
	sys->Cvar_Set( "g_medicChargeTime",Info_ValueForKey( info,"g_medicChargeTime" ) );
	sys->Cvar_Set( "g_engineerChargeTime",Info_ValueForKey( info,"g_engineerChargeTime" ) );
	sys->Cvar_Set( "g_soldierChargeTime",Info_ValueForKey( info,"g_soldierChargeTime" ) );
	sys->Cvar_Set( "g_LTChargeTime",Info_ValueForKey( info,"g_LTChargeTime" ) );
	sys->Cvar_Set( "g_redlimbotime",Info_ValueForKey( info,"g_redlimbotime" ) );
	sys->Cvar_Set( "g_bluelimbotime",Info_ValueForKey( info,"g_bluelimbotime" ) );
// jpw

	//	Q_strncpyz( cgs.redTeam, Info_ValueForKey( info, "g_redTeam" ), sizeof(cgs.redTeam) );
//	sys->Cvar_Set("g_redTeam", cgs.redTeam);
//	Q_strncpyz( cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam) );
//	sys->Cvar_Set("g_blueTeam", cgs.blueTeam);
}
...
static void CG_ParseWarmup( void ) {
	...
	if ( warmup == 0 && cg.warmup ) {
		...
		sys->S_StartLocalSound( cgs.media.countPrepareSound, CHAN_ANNOUNCER );
	}
...
static void CG_ParseFog( void ) {
	...
	if ( !token || !token[0] ) {
		...
		sys->R_SetFog( FOG_CMD_SWITCHFOG, FOG_MAP, (int)ne, 0, 0, 0, 0 );
		return;
	}
	...
	sys->R_SetFog( FOG_SERVER, (int)ne, (int)fa, r, g, b, density );
	sys->R_SetFog( FOG_CMD_SWITCHFOG, FOG_SERVER, time, 0, 0, 0, 0 );
}
...
void CG_SendMoveSpeed( animation_t *animList, int numAnims, char *modelName ) {
	...
	sys->SendMoveSpeedsToGame( 0, text );
}
...
//	sys->R_SetFog(FOG_CMD_SWITCHFOG, FOG_MAP,20,0,0,0,0);
...
//	sys->SendClientCommand(va("fogswitch %s", buff) );
	...
	sys->S_ClearLoopingSounds( qtrue );
...
//		sys->S_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
...
//		CG_VoiceChat( SAY_TELL );
	...
	if ( Q_stricmp( cmd, "remapShader" ) == 0 ) {
		if ( sys->Argc() == 4 ) {
			sys->R_RemapShader( CG_Argv( 1 ), CG_Argv( 2 ), CG_Argv( 3 ) );
		}
	}
	...
	if ( !strcmp( cmd, "mu_play" ) ) {   // has optional parameter for fade-up time
		...
		sys->S_StartBackgroundTrack( CG_Argv( 1 ), "onetimeonly", fadeTime );
		return;
	}
	...
	if ( !strcmp( cmd, "mu_fade" ) ) {
		sys->S_FadeBackgroundTrack( atof( CG_Argv( 1 ) ), atoi( CG_Argv( 2 ) ), 0 );
		return;
	}

	if ( !strcmp( cmd, "snd_fade" ) ) {
		sys->S_FadeAllSound( atof( CG_Argv( 1 ) ), atoi( CG_Argv( 2 ) ) );
		return;
	}

	if ( !strcmp( cmd, "rockandroll" ) ) {   // map loaded, game is ready to begin.
		CG_Fade( 0, 0, 0, 255, cg.time, 0 );      // go black
		sys->UI_Popup( "pregame" );                // start pregame menu
		sys->Cvar_Set( "cg_norender", "1" );    // don't render the world until the player clicks in and the 'playerstart' func has been called (g_main in G_UpdateCvars() ~ilne 949)
	...
	if ( !strcmp( cmd, "addToBuild" ) ) {
		...
		sys->FS_FOpenFile( CG_Argv( 1 ), &f, FS_READ );
		sys->FS_FCloseFile( f );
		return;
	}
...
void CG_ExecuteNewServerCommands( int latestSequence ) {
	while ( cgs.serverCommandSequence < latestSequence ) {
		if ( sys->GetServerCommand( ++cgs.serverCommandSequence ) ) {
			CG_ServerCommand();
		}
	}
}
