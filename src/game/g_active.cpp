...
void    G_TouchTriggers( gentity_t *ent ) {
	...
	for ( i = 0 ; i < num ; i++ ) {
		...
		if ( hit->s.eType == ET_ITEM ) {
			...
			if ( !sys->EntityContactCapsule( mins, maxs, hit ) ) {
				continue;
			}
		}
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	if ( !g_inactivity.integer ) {
		...
		if ( level.time > client->inactivityTime ) {
			sys->DropClient( client - level.clients, "Dropped due to inactivity" );
			return qfalse;
		}
		if ( level.time > client->inactivityTime - 10000 && !client->inactivityWarning ) {
			client->inactivityWarning = qtrue;
			sys->SendServerCommand( client - level.clients, "cp \"Ten seconds until inactivity drop!\n\"" );
		}
	}
	return qtrue;
}
...
//	vec3_t		oldOrigin;
	...
	if ( client->cameraPortal ) {
		...
		sys->LinkEntity( client->cameraPortal );
...
//	}
	...
	if (    !saveGamePending &&
		...
		sys->Cvar_Update( &g_missionStats );
...
//		pm.noFootsteps = qtrue;
	...
	if ( level.intermissionQueued != 0 && g_singlePlayer.integer ) {
		if ( level.time - level.intermissionQueued >= 1000  ) {
			...
			if ( level.time - level.intermissionQueued >= 2000 && level.time - level.intermissionQueued <= 2500 ) {
				sys->SendConsoleCommand( EXEC_APPEND, "centerview\n" );
			}
	...
	if ( monsterslick && !( ent->flags & FL_NO_MONSTERSLICK ) ) {
			...
			if ( !bogus ) {
				// when an actor dies test if tag_head is in a contents_solid and stop slide
				{
					...
					if ( sys->GetTag( ent->s.number, "tag_head", &or ) ) {
						...
						sys->Trace( &tr, start, NULL, NULL, end, ent->s.number, ( CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE | CONTENTS_TRIGGER ) );
		...
		if ( VectorLength( pm.ps->velocity ) < 100 && sys->InPVS( pm.ps->origin, g_entities[0].s.pos.trBase ) ) {
			...
			if ( sys->GetTag( ent->s.number, "tag_head", &or ) ) {
				...
				sys->Trace( &tr, src, vec3_origin, vec3_origin, or.origin, ent->s.number, MASK_SOLID );
...
void ClientThink( int clientNum ) {
	...
	sys->GetUsercmd( clientNum, &ent->client->pers.cmd );
