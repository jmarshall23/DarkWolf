...
qboolean G_ParseSpawnVars( void ) {
	...
	if ( !sys->GetEntityToken( com_token, sizeof( com_token ) ) ) {
		// end of spawn string
		return qfalse;
	}
	if ( com_token[0] != '{' ) {
	...
	while ( 1 ) {
		// parse key
		if ( !sys->GetEntityToken( keyname, sizeof( keyname ) ) ) {
...
void SP_worldspawn( void ) {
	...
	sys->SetConfigstring( CS_GAME_VERSION, GAME_VERSION );

	sys->SetConfigstring( CS_LEVEL_START_TIME, va( "%i", level.startTime ) );

	G_SpawnString( "music", "", &s );
	sys->SetConfigstring( CS_MUSIC, s );

	G_SpawnString( "message", "", &s );
	sys->SetConfigstring( CS_MESSAGE, s );              // map specific message

	sys->SetConfigstring( CS_MOTD, g_motd.string );     // message of the day

	G_SpawnString( "gravity", "800", &s );
	sys->Cvar_Set( "g_gravity", s );
