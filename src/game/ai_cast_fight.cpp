...
qboolean AICast_CheckAttack_real( cast_state_t *cs, int enemy, qboolean allowHitWorld ) {
	...
	for ( i = 0; i <= fuzzyCount; i++ ) {
		...
		if ( /*allowHitWorld &&*/ !sys->InPVS( start, end ) ) {
		...
		case AICHAR_LOPER:  // loper leap attack
			if ( cs->bs->areanum && VectorLength( cs->bs->velocity ) > 1 ) {    // if we are in a valid area, and are persuing, then leave a delay
				...
				if ( cs->enemyNum >= 0 ) {
					...
					mins[0] = 0;
					sys->Trace( &trace, g_entities[cs->entityNum].client->ps.origin, mins, cs->bs->cur_ps.maxs, g_entities[cs->enemyNum].client->ps.origin, cs->entityNum, g_entities[cs->entityNum].clipmask );
	...
	if ( cs->combatGoalTime > level.time ) {
		...
		if (    (   ( cs->attributes[CAMPER] < random() )
			...
			if (    ( cs->attributes[TACTICAL] > 0.3 + random() * 0.5 )
					&&  sys->AAS_RT_GetHidePos( cs->bs->origin, cs->bs->entitynum, cs->bs->areanum, cs->vislist[cs->enemyNum].visible_pos, cs->enemyNum, BotPointAreaNum( cs->vislist[cs->enemyNum].visible_pos ), cs->combatGoalOrigin ) ) {
