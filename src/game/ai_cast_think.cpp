...
void AICast_ProcessAIFunctions( cast_state_t *cs, float thinktime ) {
	...
	for ( i = 0; i < ( aicast_debug.integer ? MAX_AIFUNCS : 1 ); i++ )
	{
		if ( !( funcname = cs->aifunc( cs ) ) ) {
			...
			sys->BotResetAvoidReach( cs->bs->ms );    // reset avoidreach
...
void AICast_ChangeViewAngles( cast_state_t *cs, float thinktime ) {
	...
	sys->EA_View( bs->client, cs->viewangles );
}
...
//	}
	...
	sys->AAS_SetCurrentWorld( cs->aasWorldIndex );
	//
	// make sure we have a valid navigation system
	//
	if ( !sys->AAS_Initialized() ) {
		return;
	}
	//
	...
	// HACK for village2, if they are stuck, find a good position (there is a friendly guy placed inside a table)
	{
		trace_t tr;
		vec3_t org;
		sys->Trace( &tr, cs->bs->cur_ps.origin, cs->bs->cur_ps.mins, cs->bs->cur_ps.maxs, cs->bs->cur_ps.origin, cs->entityNum, CONTENTS_SOLID );
		while ( tr.startsolid ) {
			VectorCopy( cs->bs->cur_ps.origin, org );
			org[0] += 96 * crandom();
			org[1] += 96 * crandom();
			org[2] += 16 * crandom();
			sys->Trace( &tr, org, cs->bs->cur_ps.mins, cs->bs->cur_ps.maxs, org, cs->entityNum, CONTENTS_SOLID );
	...
	if ( cs->aiState == AISTATE_QUERY ) {
		...
		sys->EA_SelectWeapon( cs->bs->client, cs->weaponNum );
		//
...
void AICast_StartFrame( int time ) {
	...
	if ( sys->Cvar_VariableIntegerValue( "savegame_loading" ) ) {
		return;
	}
	...
	if ( !aicast_disable.handle ) {
		sys->Cvar_Register( &aicast_disable, "aicast_disable", "0", CVAR_CHEAT );
	} else
	{
		sys->Cvar_Update( &aicast_disable );
		if ( aicast_disable.integer ) {
			return;
		}
	}

	sys->Cvar_Update( &aicast_debug );
	sys->Cvar_Update( &aicast_debugname );
	sys->Cvar_Update( &aicast_scripts );
	...
	sys->BotLibStartFrame( (float) time / 1000 );
	//
	//
			...
			if ( ent->aiInactive == qfalse && ent->inuse ) {
				...
				if ( cs->lastThink && elapsed > 0 ) {
					...
					if (    ( highPriority && ( elapsed > 300 ) )
							...
							||  ( sys->InPVS( cs->bs->origin, g_entities[0].s.pos.trBase ) ) ) { // do pvs check last, since it's the most expensive to call
...
//	}
	...
	for ( frame = 0; frame < numframes; frame++ )
	{
		...
		pm.trace = [](trace_t* results,
			...
			int passEntityNum,
			int contentMask) {
				sys->Trace(results, start, mins, maxs, end, passEntityNum, contentMask);
			};

		pm.pointcontents = [](const vec3_t point, int passEntityNum) -> int {
			return sys->PointContents(point, passEntityNum);
			};
	...
	if ( move->groundEntityNum == ENTITYNUM_NONE ) {
		...
		sys->Trace( &tr, move->endpos, pm.mins, pm.maxs, end, pm.ps->clientNum, pm.tracemask );
...
qboolean AICast_GetAvoid( cast_state_t *cs, bot_goal_t *goal, vec3_t outpos, qboolean reverse, int blockEnt ) {
	...
	enemyVisible =  (qboolean)(( cs->enemyNum >= 0 ) &&
	...
	if ( goal ) {
		starttraveltime = sys->AAS_AreaTravelTimeToGoalArea( cs->bs->areanum, cs->bs->origin, goal->areanum, cs->travelflags );
	}
	...
	for ( yaw = -angleDiff * invert; yaw*invert <= maxYaw; yaw += inc * invert ) {
		...
		if ( cs->dangerEntity >= 0 && cs->dangerEntityValidTime >= level.time ) {
			...
			//distmoved = 99999 - sys->AAS_AreaTravelTimeToGoalArea( BotPointAreaNum(castmove.endpos), castmove.endpos, goal->areanum, cs->travelflags );
		...
		if (    ( distmoved > bestmoved )
			...
			if ( !enemyVisible || AICast_CheckAttackAtPos( cs->entityNum, cs->enemyNum, castmove.endpos, qfalse, qfalse ) ) {
				if ( !goal || ( traveltime = sys->AAS_AreaTravelTimeToGoalArea( BotPointAreaNum( castmove.endpos ), castmove.endpos, goal->areanum, cs->travelflags ) ) < ( starttraveltime + 200 ) ) {
					bestyaw = yaw;
					bestmoved = distmoved;
...
void AICast_Blocked( cast_state_t *cs, bot_moveresult_t *moveresult, int activate, bot_goal_t *goal ) {
	...
	if ( moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE ) {
		sys->EA_Jump( cs->bs->entitynum );
	}

	sys->EA_Move( cs->bs->entitynum, dir, 200 ); //400);
...
/*
	...
	sys->GetTag( cs->entityNum, "tag_head", &or );
	// move up a tad
	or.origin[2] += 3;
	...
	sys->Trace( &tr, src, vec3_origin, vec3_origin, or.origin, cs->entityNum, MASK_SOLID );
