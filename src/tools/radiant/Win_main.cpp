...
static DWORD WINAPI BSPOutputThreadProc(LPVOID pData)
{
	bspLiveOutput_t* ctx = (bspLiveOutput_t*)pData;
	if (!ctx)
	{
		return 0;
	}
	...
	for (;;)
	{
		BOOL ok = ReadFile(ctx->hReadPipe, buffer, sizeof(buffer) - 1, &bytesRead, NULL);
		if (!ok || bytesRead == 0)
		{
			break;
		}
...
void RunBsp(char* command)
{
	...
	if (g_hToolThread)
	{
		if (GetExitCodeProcess(g_hToolThread, &dwExitcode) && dwExitcode != STILL_ACTIVE)
		{
			CloseHandle(g_hToolThread);
			g_hToolThread = NULL;
		}
	}
	...
	// Use a mutable buffer because CreateProcess may modify it.
	char cmdLine[8192];
	_snprintf(cmdLine, sizeof(cmdLine) - 1, "cmd.exe /Q /C %s", sys);
	...
	// Parent never writes to the child pipe.
	CloseHandle(hWritePipe);
	hWritePipe = NULL;
	...
	bsp_process = pi.hProcess;

	Sleep(100);
	BringWindowToTop(g_qeglobals.d_hwndMain);

	bspLiveOutput_t* ctx = new bspLiveOutput_t;
	ctx->hProcess = pi.hProcess;
	ctx->hThread = pi.hThread;
	ctx->hReadPipe = hReadPipe;

	DWORD threadId = 0;
	g_hToolThread = CreateThread(NULL, 0, BSPOutputThreadProc, ctx, 0, &threadId);
	if (!g_hToolThread)
	{
		Sys_Printf("Failed to create BSP output thread\n");
