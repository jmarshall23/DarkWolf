...
//		make sure to tessellate model heads
	...
	if (strstr(name, ".mdr")) {  // try loading skeletal file
		int fileLen = -1;
		loaded = qfalse;
		...
		if (buf) {
			...
			if (ident == MDR_IDENT) {
				loaded = R_LoadMDR(mod, buf, fileLen, name);
			}
...
*/
	...
	{
		start = R_GetAnimTag((mdrHeader_t*)model->modelData, startFrame, tagName, start);
		end = R_GetAnimTag((mdrHeader_t*)model->modelData, endFrame, tagName, end);
	}
