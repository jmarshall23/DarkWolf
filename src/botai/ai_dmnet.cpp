...
int BotGoForAir( bot_state_t *bs, int tfl, bot_goal_t *ltg, float range ) {
	...
	if ( bs->lastair_time < sys->AAS_Time() - 6 ) {
		//
#ifdef DEBUG
	   ...
	   //if we can find an air goal
		if ( BotGetAirGoal( bs, &goal ) ) {
			sys->BotPushGoal( bs->gs, &goal );
			return qtrue;
		} else {
			//get a nearby goal outside the water
			while ( sys->BotChooseNBGItem( bs->gs, bs->origin, bs->inventory, tfl, ltg, range ) ) {
				sys->BotGetTopGoal( bs->gs, &goal );
				//if the goal is not in water
				if ( !( sys->AAS_PointContents( goal.origin ) & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) ) {
					return qtrue;
				}
				sys->BotPopGoal( bs->gs );
			}
...
int BotNearbyGoal( bot_state_t *bs, int tfl, bot_goal_t *ltg, float range ) {
	...
	//
	ret = sys->BotChooseNBGItem( bs->gs, bs->origin, bs->inventory, tfl, ltg, range );
	/*
	if (ret)
	{
		char buf[128];
		...
		sys->BotGetTopGoal(bs->gs, &goal);
		sys->BotGoalName(goal.number, buf, sizeof(buf));
...
int BotReachedGoal( bot_state_t *bs, bot_goal_t *goal ) {
	if ( goal->flags & GFL_ITEM ) {
		...
		if ( sys->BotTouchingGoal( bs->origin, goal ) ) {
			return qtrue;
		}
		//if the goal isn't there
		if ( sys->BotItemGoalInVisButNotVisible( bs->entitynum, bs->eye, bs->viewangles, goal ) ) {
			return qtrue;
		}
		...
		//if the bot got air
		if ( bs->lastair_time > sys->AAS_Time() - 1 ) {
			return qtrue;
		}
	} else {
		//if touching the goal
...
int BotGetItemLongTermGoal( bot_state_t *bs, int tfl, bot_goal_t *goal ) {
	...
	if ( !sys->BotGetTopGoal( bs->gs, goal ) ) {
	...
	if ( bs->ltg_time < sys->AAS_Time() ) {
		//pop the current goal from the stack
		sys->BotPopGoal( bs->gs );
		//BotAI_Print(PRT_MESSAGE, "%s: choosing new ltg\n", ClientName(bs->client, netname, sizeof(netname)));
		//choose a new goal
		//BotAI_Print(PRT_MESSAGE, "%6.1f client %d: BotChooseLTGItem\n", sys->AAS_Time(), bs->client);
		if ( sys->BotChooseLTGItem( bs->gs, bs->origin, bs->inventory, tfl ) ) {
			/*
			char buf[128];
			//get the goal at the top of the stack
			sys->BotGetTopGoal(bs->gs, goal);
		...
		//get the goal at the top of the stack
		return sys->BotGetTopGoal( bs->gs, goal );
	}
	return qtrue;
}
...
int BotGetLongTermGoal( bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal ) {
	...
	if ( bs->ltgtype == LTG_TEAMHELP && !retreat ) {
		...
		if ( bs->teammessage_time && bs->teammessage_time < sys->AAS_Time() ) {
		...
		if ( bs->teamgoal_time < sys->AAS_Time() ) {
			bs->ltgtype = 0;
		}
		//if the team mate IS visible for quite some time
		if ( bs->teammatevisible_time < sys->AAS_Time() - 10 ) {
			bs->ltgtype = 0;
		}
		...
		if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate ) ) {
			...
			bs->teammatevisible_time = sys->AAS_Time();
		}
		//if the entity information is valid (entity in PVS)
		if ( entinfo.valid ) {
			areanum = BotPointAreaNum( entinfo.origin );
			if ( areanum && sys->AAS_AreaReachability( areanum ) ) {
				//update team goal
	...
	if ( bs->ltgtype == LTG_TEAMACCOMPANY && !retreat ) {
		...
		if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate ) ) {
			...
			if ( VectorLength( dir ) < bs->formation_dist ) {
				...
				if ( bs->attackcrouch_time < sys->AAS_Time() - 5 ) {
					croucher = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_CROUCHER, 0, 1 );
					if ( random() < bs->thinktime * croucher ) {
						bs->attackcrouch_time = sys->AAS_Time() + 5 + croucher * 15;
					}
				}
				//don't crouch when swimming
				if ( sys->AAS_Swimming( bs->origin ) ) {
					bs->attackcrouch_time = sys->AAS_Time() - 1;
				}
				//if not arrived yet or arived some time ago
				if ( bs->arrive_time < sys->AAS_Time() - 2 ) {
					//if not arrived yet
					if ( !bs->arrive_time ) {
						sys->EA_Gesture( bs->client );
						BotAI_BotInitialChat( bs, "accompany_arrive", EasyClientName( bs->teammate, netname, sizeof( netname ) ), NULL );
						sys->BotEnterChat( bs->cs, bs->client, CHAT_TEAM );
						bs->arrive_time = sys->AAS_Time();
					}
					//if the bot wants to crouch
					else if ( bs->attackcrouch_time > sys->AAS_Time() ) {
						sys->EA_Crouch( bs->client );
					}
				...
				if ( BotGoForAir( bs, bs->tfl, &bs->teamgoal, 400 ) ) {
					sys->BotResetLastAvoidReach( bs->ms );
					//get the goal at the top of the stack
					//sys->BotGetTopGoal(bs->gs, &tmpgoal);
					//sys->BotGoalName(tmpgoal.number, buf, 144);
					//BotAI_Print(PRT_MESSAGE, "new nearby goal %s\n", buf);
					//time the bot gets to pick up the nearby goal item
					bs->nbg_time = sys->AAS_Time() + 8;
					AIEnter_Seek_NBG( bs );
					return qfalse;
				}
				//
		...
		if ( bs->teammatevisible_time < sys->AAS_Time() - 60 ) {
	...
	if ( bs->ltgtype == LTG_DEFENDKEYAREA ) {
		if ( sys->AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin,
	...
	if ( bs->ltgtype == LTG_DEFENDKEYAREA && !retreat &&
		 bs->defendaway_time < sys->AAS_Time() ) {
		//check for bot typing status message
		if ( bs->teammessage_time && bs->teammessage_time < sys->AAS_Time() ) {
			sys->BotGoalName( bs->teamgoal.number, buf, sizeof( buf ) );
		...
		if ( VectorLength( dir ) < 70 ) {
			sys->BotResetAvoidReach( bs->ms );
			bs->defendaway_time = sys->AAS_Time() + 2 + 5 * random();
	...
	if ( ( bs->ltgtype == LTG_CAMP || bs->ltgtype == LTG_CAMPORDER ) && !retreat ) {
		...
		if ( VectorLength( dir ) < 60 ) {
			...
			if ( sys->PointContents( bs->eye,bs->entitynum ) & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) {
				if ( bs->ltgtype == LTG_CAMPORDER ) {
					BotAI_BotInitialChat( bs, "camp_stop", NULL );
					sys->BotEnterChat( bs->cs, bs->client, CHAT_TEAM );
				}
	...
	if ( bs->ltgtype == LTG_PATROL && !retreat ) {
		...
		//if the bot touches the current goal
		if ( sys->BotTouchingGoal( bs->origin, &bs->curpatrolpoint->goal ) ) {
	...
	if ( bs->ltgtype == LTG_RUSHBASE && bs->rushbaseaway_time < sys->AAS_Time() ) {
		switch ( BotCTFTeam( bs ) ) {
		...
		if ( sys->BotTouchingGoal( bs->origin, goal ) ) {
			...
			if ( BotCTFCarryingFlag( bs ) ) {
				sys->BotResetAvoidReach( bs->ms );
				bs->rushbaseaway_time = sys->AAS_Time() + 5 + 10 * random();
...
int BotLongTermGoal( bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal ) {
	...
	if ( bs->lead_time > 0 && !retreat ) {
		...
		if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->lead_teammate ) ) {
			bs->leadvisible_time = sys->AAS_Time();
		}
		//if the team mate is not visible for 1 seconds
		if ( bs->leadvisible_time < sys->AAS_Time() - 1 ) {
			bs->leadbackup_time = sys->AAS_Time() + 2;
		}
		...
		if ( bs->leadbackup_time > sys->AAS_Time() ) {
			if ( bs->leadmessage_time < sys->AAS_Time() - 20 ) {
				BotAI_BotInitialChat( bs, "followme", EasyClientName( bs->lead_teammate, teammate, sizeof( teammate ) ), NULL );
				sys->BotEnterChat( bs->cs, bs->client, CHAT_TEAM );
				bs->leadmessage_time = sys->AAS_Time();
			}
...
void AIEnter_Intermission( bot_state_t *bs ) {
	...
	if ( BotChat_EndLevel( bs ) ) {
		sys->BotEnterChat( bs->cs, bs->client, bs->chatto );
	}
...
void AIEnter_Stand( bot_state_t *bs ) {
	...
	bs->standfindenemy_time = sys->AAS_Time() + 1;
...
int AINode_Stand( bot_state_t *bs ) {
	...
	if ( bs->lastframe_health > bs->inventory[INVENTORY_HEALTH] ) {
		if ( BotChat_HitTalking( bs ) ) {
			bs->standfindenemy_time = sys->AAS_Time() + BotChatTime( bs ) + 0.1;
			bs->stand_time = sys->AAS_Time() + BotChatTime( bs ) + 0.1;
		}
	}
	if ( bs->standfindenemy_time < sys->AAS_Time() ) {
	...
	sys->EA_Talk( bs->client );
	if ( bs->stand_time < sys->AAS_Time() ) {
		sys->BotEnterChat( bs->cs, bs->client, bs->chatto );
		AIEnter_Seek_LTG( bs );
		return qfalse;
	}
...
void AIEnter_Respawn( bot_state_t *bs ) {
	...
	if ( BotChat_Death( bs ) ) {
		bs->respawn_time = sys->AAS_Time() + BotChatTime( bs );
		bs->respawnchat_time = sys->AAS_Time();
	} else {
		bs->respawn_time = sys->AAS_Time() + 1 + random();
...
int AINode_Respawn( bot_state_t *bs ) {
	if ( bs->respawn_wait ) {
		if ( !BotIsDead( bs ) ) {
			...
			sys->EA_Respawn( bs->client );
		}
	} else if ( bs->respawn_time < sys->AAS_Time() )     {
	...
	if ( bs->respawnchat_time && bs->respawnchat_time < sys->AAS_Time() - 0.5 ) {
		sys->EA_Talk( bs->client );
	}
	//
	return qtrue;
...
int AINode_Seek_ActivateEntity( bot_state_t *bs ) {
	...
	//if the bot touches the current goal
	else if ( sys->BotTouchingGoal( bs->origin, goal ) ) {
		BotChooseWeapon( bs );
#ifdef DEBUG
	...
	//
	if ( bs->activate_time < sys->AAS_Time() ) {
		AIEnter_Seek_NBG( bs );
		return qfalse;
	}
	...
	sys->BotMoveToGoal( &moveresult, bs->ms, goal, bs->tfl );
	...
	//if waiting for something
		...
		if ( sys->BotMovementViewTarget( bs->ms, goal, bs->tfl, 300, target ) ) {
			VectorSubtract( target, bs->origin, dir );
			vectoangles( dir, bs->ideal_viewangles );
		} else {
...
void AIEnter_Seek_NBG( bot_state_t *bs ) {
	...
	if ( sys->BotGetTopGoal( bs->gs, &goal ) ) {
		sys->BotGoalName( goal.number, buf, 144 );
...
int AINode_Seek_NBG( bot_state_t *bs ) {
	...
	if ( bs->nbg_time < sys->AAS_Time() ) {
		...
		bs->check_time = sys->AAS_Time() + 0.05;
		//go back to seek ltg
		AIEnter_Seek_LTG( bs );
		return qfalse;
	}
	...
	//if waiting for something
		...
		if ( !sys->BotGetSecondGoal( bs->gs, &goal ) ) {
			sys->BotGetTopGoal( bs->gs, &goal );
		}
		if ( sys->BotMovementViewTarget( bs->ms, &goal, bs->tfl, 300, target ) ) {
			VectorSubtract( target, bs->origin, dir );
...
int AINode_Seek_LTG( bot_state_t *bs ) {
	...
	bs->enemy = -1;
	//
	if ( bs->killedenemy_time > sys->AAS_Time() - 2 ) {
	...
	if ( bs->check_time < sys->AAS_Time() ) {
		bs->check_time = sys->AAS_Time() + 0.5;
	   ...
	   //
		if ( BotNearbyGoal( bs, bs->tfl, &goal, range ) ) {
			...
			//BotAI_Print(PRT_MESSAGE, "new nearby goal %s\n", buf);
			//time the bot gets to pick up the nearby goal item
			bs->nbg_time = sys->AAS_Time() + 4 + range * 0.01;
			AIEnter_Seek_NBG( bs );
			return qfalse;
		}
	}
...
int AINode_Battle_Fight( bot_state_t *bs ) {
	...
	if ( bs->enemydeath_time ) {
		if ( bs->enemydeath_time < sys->AAS_Time() - 1.5 ) {
		...
		if ( EntityIsDead( &entinfo ) ) {
			bs->enemydeath_time = sys->AAS_Time();
		}
	}
...
int AINode_Battle_Chase( bot_state_t *bs ) {
	...
	if ( !bs->chase_time || bs->chase_time < sys->AAS_Time() - 10 ) {
		AIEnter_Seek_LTG( bs );
		return qfalse;
	}
	//check for nearby goals periodicly
	if ( bs->check_time < sys->AAS_Time() ) {
		bs->check_time = sys->AAS_Time() + 1;
		range = 150;
		//
		if ( BotNearbyGoal( bs, bs->tfl, &goal, range ) ) {
			//the bot gets 5 seconds to pick up the nearby goal item
			bs->nbg_time = sys->AAS_Time() + 0.1 * range + 1;
			sys->BotResetLastAvoidReach( bs->ms );
			AIEnter_Battle_NBG( bs );
			return qfalse;
		}
	...
	if ( moveresult.flags & ( MOVERESULT_MOVEMENTVIEWSET | MOVERESULT_MOVEMENTVIEW | MOVERESULT_SWIMVIEW ) ) {
		...
		if ( bs->chase_time > sys->AAS_Time() - 2 ) {
			BotAimAtEnemy( bs );
		} else {
			if ( sys->BotMovementViewTarget( bs->ms, &goal, bs->tfl, 300, target ) ) {
				VectorSubtract( target, bs->origin, dir );
...
int AINode_Battle_Retreat( bot_state_t *bs ) {
	...
	if ( BotEntityVisible( bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy ) ) {
		bs->enemyvisible_time = sys->AAS_Time();
		//update the reachability area and origin if possible
		areanum = BotPointAreaNum( entinfo.origin );
		if ( areanum && sys->AAS_AreaReachability( areanum ) ) {
			VectorCopy( entinfo.origin, bs->lastenemyorigin );
	...
	if ( bs->enemyvisible_time < sys->AAS_Time() - 4 ) {
		AIEnter_Seek_LTG( bs );
		return qfalse;
	}
	//else if the enemy is NOT visible
	else if ( bs->enemyvisible_time < sys->AAS_Time() ) {
		//if there is another enemy
		if ( BotFindEnemy( bs, -1 ) ) {
			AIEnter_Battle_Fight( bs );
			return qfalse;
	...
	if ( bs->check_time < sys->AAS_Time() ) {
	   ...
	   //
		if ( BotNearbyGoal( bs, bs->tfl, &goal, range ) ) {
			sys->BotResetLastAvoidReach( bs->ms );
			//time the bot gets to pick up the nearby goal item
			bs->nbg_time = sys->AAS_Time() + range / 100 + 1;
			AIEnter_Battle_NBG( bs );
			return qfalse;
		}
	}
