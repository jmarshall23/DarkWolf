...
long RllDecodeStereoToStereo(unsigned char* from, short* to, unsigned int size, char signedOutput, unsigned short flag) {
    ...
    if (signedOutput) {
        prevL = (short)((flag & 0xff00) - 0x8000);
        prevR = (short)(((flag & 0x00ff) << 8) - 0x8000);
    }
    else {
        prevL = (short)(flag & 0xff00);
        prevR = (short)((flag & 0x00ff) << 8);
    }
...
long RllDecodeStereoToMono(unsigned char* from, short* to, unsigned int size, char signedOutput, unsigned short flag) {
    ...
    for (z = 0; z < size; z += 1) {
        prevL = (short)(prevL + cin.sqrTable[from[z * 2]]);
        prevR = (short)(prevR + cin.sqrTable[from[z * 2 + 1]]);
...
static void blitVQQuad32fs(byte** status, unsigned char* data) {
    ...
    do {
        if (!newd) {
            newd = 7;
            celdata = (unsigned short)(data[0] + data[1] * 256);
            data += 2;
        }
        else {
            newd--;
...
static void recurseQuad(long startX, long startY, long quadSize, long xOff, long yOff) {
    ...
    lowx = lowy = 0;
    bigx = (long)cinTable[currentHandle].xsize;
    bigy = (long)cinTable[currentHandle].ysize;
    ...
    if ((startX >= lowx) &&
        ...
        useY = startY;
        scroff = cin.linbuf +
            (useY + ((cinTable[currentHandle].CIN_HEIGHT - bigy) >> 1) + yOff) * cinTable[currentHandle].samplesPerLine +
...
static void setupQuad(long xOff, long yOff) {
    ...
    if (xOff == cin.oldXOff &&
        yOff == cin.oldYOff &&
        cinTable[currentHandle].ysize == (unsigned int)cin.oldysize &&
        cinTable[currentHandle].xsize == (unsigned int)cin.oldxsize) {
        return;
    }

    cin.oldXOff = xOff;
    cin.oldYOff = yOff;
    cin.oldysize = (long)cinTable[currentHandle].ysize;
    cin.oldxsize = (long)cinTable[currentHandle].xsize;

    numQuadCels = (cinTable[currentHandle].CIN_WIDTH * cinTable[currentHandle].CIN_HEIGHT) / 16;
    numQuadCels += numQuadCels / 4 + numQuadCels / 16;
    numQuadCels += 64;

    numQuadCels = ((long)cinTable[currentHandle].xsize * (long)cinTable[currentHandle].ysize) / 16;
...
static void readQuadInfo(byte* qData) {
    ...
    cinTable[currentHandle].CIN_HEIGHT = (int)cinTable[currentHandle].ysize;
    cinTable[currentHandle].CIN_WIDTH = (int)cinTable[currentHandle].xsize;
...
static void RoQPrepMcomp(long xoff, long yoff) {
    ...
    for (y = 0; y < 16; y++) {
        temp2 = (intptr_t)(y + yoff - 8) * (intptr_t)i;
        for (x = 0; x < 16; x++) {
            temp = (intptr_t)(x + xoff - 8) * (intptr_t)j;
    ...
    case ROQ_QUAD_VQ:
        ...
        if (cinTable[currentHandle].numQuads == 0) {
            Com_Memcpy(cin.linbuf + cinTable[currentHandle].screenDelta,
                cin.linbuf,
                cinTable[currentHandle].samplesPerLine * (long)cinTable[currentHandle].ysize);
        }
...
e_status CIN_RunCinematic(int handle) {
    ...
    cinTable[currentHandle].tfps =
        (((CL_ScaledMilliseconds() - (int)cinTable[currentHandle].startTime) * cinTable[currentHandle].roqFPS) / 1000);

    start = (int)cinTable[currentHandle].startTime;
    ...
    cinTable[currentHandle].lastTime = (unsigned int)thisTime;
...
void CIN_DrawCinematic(int handle) {
    ...
    if (cinTable[handle].letterBox) {
        ...
        barheight = ((float)LETTERBOX_OFFSET / 480.0f) * vh;
    ...
    if (cinTable[handle].dirty &&
        ...
        if (xm == 2 && ym == 2) {
            ...
            for (iy = 0; iy < 256; iy++) {
                ...
                for (ix = 0; ix < 2048; ix += 8) {
                    for (ic = ix; ic < (ix + 4); ic++) {
                        *bc2 = (byte)((bc3[iiy + ic] + bc3[iiy + 4 + ic] + bc3[iiy + 2048 + ic] + bc3[iiy + 2048 + 4 + ic]) >> 2);
                        bc2++;
                    }
                }
            }
            ...
            for (iy = 0; iy < 256; iy++) {
                ...
                for (ix = 0; ix < 2048; ix += 8) {
                    for (ic = ix; ic < (ix + 4); ic++) {
                        *bc2 = (byte)((bc3[iiy + ic] + bc3[iiy + 4 + ic]) >> 1);
                        bc2++;
                    }
                }
            }
