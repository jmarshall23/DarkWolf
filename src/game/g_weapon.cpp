...
//	vec3_t		pforward, eforward;
	...
	sys->Trace( &tr, muzzleTrace, NULL, NULL, end, ent->s.number, MASK_SHOT );
...
//	int			mod = MOD_KNIFE;
	...
	sys->Trace( &tr, muzzleTrace, NULL, NULL, end, ent->s.number, MASK_SHOT | CONTENTS_TRIGGER );
...
// JPW NERVE
			if ( ent->client->sess.sessionTeam == TEAM_RED ) {
				sys->SendServerCommand( -1, "cp \"Axis engineer disarmed a det charge!\n\"" );
			} else {
				sys->SendServerCommand( -1, "cp \"Allied engineer disarmed a det charge!\n\"" );
			}
// jpw
		}
		...
		if ( traceEnt->health >= 255 ) {
			...
			sys->SendServerCommand( ent - g_entities, "cp \"You have repaired the MG42!\n\"" );
		} else {
			traceEnt->health += 3;
		}
	}
...
void weapon_callAirStrike( gentity_t *ent ) {
	...
	sys->Trace( &tr, ent->s.pos.trBase, NULL, NULL, bomboffset, ent->s.number, MASK_SHOT );
...
*/
	for ( i = 0; i < NUMBOMBS; i++ ) {
		...
		sys->Trace( &tr, bomboffset, NULL, NULL, fallaxis, ent->s.number, MASK_SHOT );
		if ( tr.fraction != 1.0 ) {
...
void Bullet_Fire_Extended( gentity_t *source, gentity_t *attacker, vec3_t start, vec3_t end, float spread, int damage, int recursion ) {
	...
	sys->Trace( &tr, start, NULL, NULL, end, source->s.number, MASK_SHOT );
//	sys->Trace (&tr, start, NULL, NULL, end, ENTITYNUM_NONE, MASK_SHOT);
	...
	*/
...

	{
...

		{
			...
			sys->Trace( &tr, viewpos, NULL, NULL, tosspos, ent->s.number, MASK_SHOT );
...
qboolean VenomPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	sys->Trace( &tr, start, NULL, NULL, end, ent->s.number, MASK_SHOT );
	traceEnt = &g_entities[ tr.entityNum ];
...
//		VectorCopy( ent->s.pos.trBase, viewpos );
//		viewpos[2] += ent->client->ps.viewheight;
//		sys->Trace (&tr, viewpos, NULL, NULL, muzzleEffect, ent->s.number, MASK_SHOT);
//		if(tr.fraction < 1) {	// oops, bad launch spot
