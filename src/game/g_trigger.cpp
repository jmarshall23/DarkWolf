...
void InitTrigger( gentity_t *self ) {
	...
	self->r.contents = CONTENTS_TRIGGER;        // replaces the -1 from sys->SetBrushModel
...
void Enable_Trigger_Touch( gentity_t *ent ) {
	...
	} else
	{
		...
		sys->Trace( &tr, targ->client->ps.origin, targ->r.mins, targ->r.maxs, targ->client->ps.origin, targ->s.number, mask );
...
void gas_touch( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	sys->Trace( &tr, ent->r.currentOrigin, NULL, NULL, other->r.currentOrigin, ent->s.number, MASK_SHOT );
