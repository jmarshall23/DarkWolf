...
static void CG_spWin_f( void) {
	...
	sys->Cvar_Set("cg_thirdPersonAngle", "0");
	sys->Cvar_Set("cg_thirdPersonRange", "100");
//	CG_AddBufferedSound(cgs.media.winnerSound);
	//sys->S_StartLocalSound(cgs.media.winnerSound, CHAN_ANNOUNCER);
...
//	CG_AddBufferedSound(cgs.media.loserSound);
	//sys->S_StartLocalSound(cgs.media.loserSound, CHAN_ANNOUNCER);
...
static void CG_PickTeam_f( void ) {
	...
	// set map title
	sys->Cvar_VariableStringBuffer( "sv_mapname", buf, sizeof( buf ) );
	sys->Cvar_Set( "mp_mapTitle", "MAP" ); //buf );

	// set map description
	s = CG_ConfigString( CS_MULTI_MAPDESC );
	if ( s )
		sys->Cvar_Set( "mp_mapDesc", s );
...
static void CG_PickPlayer_f( void ) {
	...
	sys->UI_Popup( "UIMENU_WM_PICKPLAYER" );
}
*/
...
//	sys->AddCommand ("votell");
	...
	sys->AddCommand( "noclip" );
	sys->AddCommand( "team" );
	sys->AddCommand( "follow" );
	sys->AddCommand( "levelshot" );
	sys->AddCommand( "addbot" );
...
//	sys->AddCommand ("teamtask");
	...
	sys->AddCommand( "setCameraOrigin" );
