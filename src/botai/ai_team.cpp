...
int BotClientTravelTimeToGoal( int client, bot_goal_t *goal ) {
	...
	return sys->AAS_AreaTravelTimeToGoalArea( areanum, ps.origin, goal->areanum, TFL_DEFAULT );
}
...
void BotSayTeamOrder( bot_state_t *bs, int toclient ) {
	...
	if ( bs->client == toclient ) {
		...
		sys->BotGetChatMessage( bs->cs, buf, sizeof( buf ) );
		ClientName( bs->client, name, sizeof( name ) );
		Com_sprintf( teamchat, sizeof( teamchat ), "(%s): %s", name, buf );
		sys->BotQueueConsoleMessage( bs->cs, CMS_CHAT, teamchat );
	} else {
...
void BotTeamAI( bot_state_t *bs ) {
	...
	if ( !BotValidTeamLeader( bs ) ) {
		...
		if ( !bs->askteamleader_time && !bs->becometeamleader_time ) {
			if ( bs->entergame_time + 10 > sys->AAS_Time() ) {
				bs->askteamleader_time = sys->AAS_Time() + 5 + random() * 10;
			} else {
				bs->becometeamleader_time = sys->AAS_Time() + 5 + random() * 10;
			}
		}
		if ( bs->askteamleader_time && bs->askteamleader_time < sys->AAS_Time() ) {
			//if asked for a team leader and no repsonse
			BotAI_BotInitialChat( bs, "whoisteamleader", NULL );
			sys->BotEnterChat( bs->cs, bs->client, CHAT_TEAM );
			bs->askteamleader_time = 0;
			bs->becometeamleader_time = sys->AAS_Time() + 15 + random() * 10;
		}
		if ( bs->becometeamleader_time && bs->becometeamleader_time < sys->AAS_Time() ) {
	...
	case GT_TEAM:
	{
		if ( bs->numteammates != numteammates || bs->forceorders ) {
			bs->teamgiveorders_time = sys->AAS_Time();
			bs->numteammates = numteammates;
			bs->forceorders = qfalse;
		}
		//if it's time to give orders
		if ( bs->teamgiveorders_time < sys->AAS_Time() - 5 ) {
			BotTeamOrders( bs );
			//
	...
	case GT_CTF:
	{
		...
		//if it's time to give orders
		if ( bs->teamgiveorders_time && bs->teamgiveorders_time < sys->AAS_Time() - 3 ) {
			//
			if ( BotCTFTeam( bs ) == CTF_TEAM_RED ) {
