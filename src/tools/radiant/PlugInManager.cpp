...
qtexture_t* WINAPI QERApp_Texture_ForName (const char *name)
{
	...
	HDC pluginHDC = wglGetCurrentDC();
	HGLRC pluginHGLRC = (HGLRC)wglGetCurrentContext();
	wglMakeCurrent( g_qeglobals.d_hdcBase, g_qeglobals.d_hglrcBase );
	qtexture_t* qtex = Texture_ForName( name );
	return qtex;
	wglMakeCurrent( pluginHDC, pluginHGLRC );
}
...
int WINAPI QERApp_RequestInterface( REFGUID refGUID, LPVOID pInterface )
{
	if (IsEqualGUID( refGUID, QERQglTable_GUID ))
	{
		...
		pQglTable->m_pfn_glAlphaFunc = (PFN_QGLALPHAFUNC)glAlphaFunc;
		pQglTable->m_pfn_glBegin = (PFN_QGLBEGIN)glBegin;
		pQglTable->m_pfn_glBindTexture = (PFN_QGLBINDTEXTURE)glBindTexture;
		pQglTable->m_pfn_glBlendFunc = (PFN_QGLBLENDFUNC)glBlendFunc;
		pQglTable->m_pfn_glCallList = (PFN_QGLCALLLIST)glCallList;
		pQglTable->m_pfn_glCallLists = (PFN_QGLCALLLISTS)glCallLists;
		pQglTable->m_pfn_glClear = (PFN_QGLCLEAR)glClear;
		pQglTable->m_pfn_glClearColor = (PFN_QGLCLEARCOLOR)glClearColor;
		pQglTable->m_pfn_glClearDepth = (PFN_QGLCLEARDEPTH)glClearDepth;
		pQglTable->m_pfn_glColor3f = (PFN_QGLCOLOR3F)glColor3f;
		pQglTable->m_pfn_glColor4f = (PFN_QGLCOLOR4F)glColor4f;
		pQglTable->m_pfn_glCullFace = (PFN_QGLCULLFACE)glCullFace;
		pQglTable->m_pfn_glDisable = (PFN_QGLDISABLE)glDisable;
		pQglTable->m_pfn_glDeleteLists = (PFN_QGLDELETELISTS)glDeleteLists;
		pQglTable->m_pfn_glEnable = (PFN_QGLENABLE)glEnable;
		pQglTable->m_pfn_glEnd = (PFN_QGLEND)glEnd;
		pQglTable->m_pfn_glEndList = (PFN_QGLENDLIST)glEndList;
		pQglTable->m_pfn_glGenLists = (PFN_QGLGENLISTS)glGenLists;
		pQglTable->m_pfn_glListBase = (PFN_QGLLISTBASE)glListBase;
		pQglTable->m_pfn_glLoadIdentity = (PFN_QGLLOADIDENTITY)glLoadIdentity;
		pQglTable->m_pfn_glMatrixMode = (PFN_QGLMATRIXMODE)glMatrixMode;
		pQglTable->m_pfn_glNewList = (PFN_QGLNEWLIST)glNewList;
		pQglTable->m_pfn_glNormal3f = (PFN_QGLNORMAL3F)glNormal3f;
		pQglTable->m_pfn_glOrtho = (PFN_QGLORTHO)glOrtho;
		pQglTable->m_pfn_glPointSize = (PFN_QGLPOINTSIZE)glPointSize;
		pQglTable->m_pfn_glPolygonMode = (PFN_QGLPOLYGONMODE)glPolygonMode;
		pQglTable->m_pfn_glPopMatrix = (PFN_QGLPOPMATRIX)glPopMatrix;
		pQglTable->m_pfn_glPushMatrix = (PFN_QGLPUSHMATRIX)glPushMatrix;
		pQglTable->m_pfn_glRotated = (PFN_QGLROTATED)glRotated;
		pQglTable->m_pfn_glRotatef = (PFN_QGLROTATEF)glRotatef;
		pQglTable->m_pfn_glScalef = (PFN_QGLSCALEF)glScalef;
		pQglTable->m_pfn_glTexCoord2f = (PFN_QGLTEXCOORD2F)glTexCoord2f;
		pQglTable->m_pfn_glTexEnvf = (PFN_QGLTEXENVF)glTexEnvf;
		pQglTable->m_pfn_glTexGenf = (PFN_QGLTEXGENF)glTexGenf;
		pQglTable->m_pfn_glTexParameterf = (PFN_QGLTEXPARAMETERF)glTexParameterf;
		pQglTable->m_pfn_glTranslated = (PFN_QGLTRANSLATED)glTranslated;
		pQglTable->m_pfn_glTranslatef = (PFN_QGLTRANSLATEF)glTranslatef;
		pQglTable->m_pfn_glVertex2f = (PFN_QGLVERTEX2F)glVertex2f;
		pQglTable->m_pfn_glVertex3f = (PFN_QGLVERTEX3F)glVertex3f;
		pQglTable->m_pfn_glViewport = (PFN_QGLVIEWPORT)glViewport;
