...
qboolean SpotWouldTelefrag( gentity_t *spot ) {
	...
	num = sys->EntitiesInBox( mins, maxs, touch, MAX_GENTITIES );
...
void CopyToBodyQue( gentity_t *ent ) {
	...
	contents = sys->PointContents( ent->s.origin, -1 );
...
void limbo( gentity_t *ent ) {
	...
	if ( !( ent->client->ps.pm_flags & PMF_LIMBO ) ) {
		...
		contents = sys->PointContents( ent->r.currentOrigin, -1 ); // drop stuff
...
//		if (reloading || saveGamePending) {
		...
		if ( !( ent->r.svFlags & SVF_CASTAI ) ) {
			...
			sys->SetConfigstring( CS_SCREENFADE, va( "1 %i 4000", level.time + 2000 ) );
//			reloading = qtrue;
...
//			level.reloadDelayTime = level.time + 1500;
			...
			sys->SendServerCommand( -1, va( "snd_fade 0 %d", 6000 ) );  // fade sound out
...
qboolean G_ParseAnimationFiles( char *modelname, gclient_t *cl ) {
	...
	if ( g_gametype.integer == GT_SINGLE_PLAYER && g_entities[0].client && g_entities[0].client->pers.connected == CON_CONNECTED ) {
		sys->SendServerCommand( 0, va( "mvspd %s", modelname ) );
	}
...
The game can override any of the settings and call sys->SetUserinfo
if desired.
============
*/
...
//----(SA) added this for head separation
	...
	if ( client->pers.connected == CON_CONNECTED ) {
		if ( strcmp( oldname, client->pers.netname ) ) {
			sys->SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " renamed to %s\n\"", oldname,
											client->pers.netname ) );
		}
	}
...
//----(SA) end
	...
	sys->SetConfigstring( CS_PLAYERS + clientNum, s );
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	if ( firstTime ) {
		...
		if ( !ent->r.svFlags & SVF_CASTAI ) {
			// done.
			sys->SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " connected\n\"", client->pers.netname ) );
		}
	}
...
void ClientBegin( int clientNum ) {
	...
	if ( client->sess.sessionTeam != TEAM_SPECTATOR ) {
		...
		if ( g_gametype.integer != GT_TOURNAMENT ) {
			...
			if ( !ent->r.svFlags & SVF_CASTAI ) {
				// done.
				sys->SendServerCommand( -1, va( "print \"%s" S_COLOR_WHITE " entered the game\n\"", client->pers.netname ) );
			}
		}
	}
...
// SetWolfSpawnWeapons() now adds medic team bonus and stores in ps.stats[STAT_MAX_HEALTH].
	...
	sys->GetUsercmd( client - level.clients, &ent->client->pers.cmd );
...
call sys->DropClient(), which will call this and do
...
void ClientDisconnect( int clientNum ) {
	...
	sys->SetConfigstring( CS_PLAYERS + clientNum, "" );
