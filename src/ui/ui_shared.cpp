...
#ifdef CGAME
#define MEM_POOL_SIZE  128 * 1024 * 8
#else
#define MEM_POOL_SIZE  1024 * 1024 * 8
#endif

static char memoryPool[MEM_POOL_SIZE];
static intptr_t allocPoint, outOfMemory;
...
void *UI_Alloc( int size ) {
	...
	if ( allocPoint + size > MEM_POOL_SIZE ) {
		...
		//DC->sys->Print(S_COLOR_YELLOW"WARNING: UI Out of Memory!\n");
		return NULL;
	}
...
void PC_SourceWarning( int handle, char *format, ... ) {
	...
	filename[0] = '\0';
	line = 0;
	sys->PC_SourceFileAndLine( handle, filename, &line );
...
qboolean Item_TextField_HandleKey( itemDef_t *item, int key ) {
	...
	if ( item->cvar ) {
		...
		if ( key & K_CHAR_FLAG ) {
			...
			if ( key == K_HOME || key == K_KP_HOME ) { // || ( tolower(key) == 'a' && sys->Key_IsDown( K_CTRL ) ) ) {
				item->cursorPos = 0;
				editPtr->paintOffset = 0;
				return qtrue;
			}

			if ( key == K_END || key == K_KP_END ) { // ( tolower(key) == 'e' && sys->Key_IsDown( K_CTRL ) ) ) {
				item->cursorPos = len;
...
//float			sys->Cvar_VariableValue( const char *var_name );
