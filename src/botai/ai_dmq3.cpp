...
void BotCTFRetreatGoals( bot_state_t *bs ) {
	...
	if ( BotCTFCarryingFlag( bs ) ) {
		...
		if ( bs->ltgtype != LTG_RUSHBASE ) {
			...
			bs->teamgoal_time = sys->AAS_Time() + CTF_RUSHBASE_TIME;
...
void BotCTFSeekGoals( bot_state_t *bs ) {
	...
	if ( bs->ctfroam_time > sys->AAS_Time() ) {
		return;
	}
	...
	bs->teammessage_time = sys->AAS_Time() + 2 * random();
	...
	if ( rnd < 0.33 && ctf_redflag.areanum && ctf_blueflag.areanum ) {
		...
		bs->teamgoal_time = sys->AAS_Time() + CTF_GETFLAG_TIME;
		...
		bs->teamgoal_time = sys->AAS_Time() + TEAM_DEFENDKEYAREA_TIME;
		bs->defendaway_time = 0;
	} else {
		bs->ltgtype = 0;
		//set the time the bot will stop roaming
		bs->ctfroam_time = sys->AAS_Time() + CTF_ROAM_TIME;
	}
#ifdef DEBUG
	BotPrintTeamGoal( bs );
#endif //DEBUG
...
int BotPointAreaNum( vec3_t origin ) {
	...
	areanum = sys->AAS_PointAreaNum( origin );
	if ( areanum ) {
		return areanum;
	}
	VectorCopy( origin, end );
	end[2] += 10;
	numareas = sys->AAS_TraceAreas( origin, end, areas, NULL, 1 );
	if ( numareas > 0 ) {
		return areas[0];
	}
...
char *ClientName( int client, char *name, int size ) {
	...
	sys->GetConfigstring( CS_PLAYERS + client, buf, sizeof( buf ) );
...
void BotChooseWeapon( bot_state_t *bs ) {
	...
	if (    bs->cur_ps.weaponstate == WEAPON_RAISING ||
		...
		newweaponnum = sys->BotChooseBestFightWeapon( bs->ws, bs->inventory );
		if ( bs->weaponnum != newweaponnum ) {
			bs->weaponchange_time = sys->AAS_Time();
		}
...
void BotSetupForMovement( bot_state_t *bs ) {
	...
	//
	sys->BotInitMoveState( bs->ms, &initmove );
}
...
qboolean BotIsObserver( bot_state_t *bs ) {
	...
	sys->GetConfigstring( CS_PLAYERS + bs->client, buf, sizeof( buf ) );
...
qboolean BotInLava( bot_state_t *bs ) {
	...
	feet[2] -= 23;
	return (qboolean)( sys->AAS_PointContents( feet ) & CONTENTS_LAVA );
}
...
qboolean BotInSlime( bot_state_t *bs ) {
	...
	VectorCopy( bs->origin, feet );
	feet[2] -= 23;
	return (qboolean)( sys->AAS_PointContents( feet ) & CONTENTS_SLIME );
}
...
int BotCanAndWantsToRocketJump( bot_state_t *bs ) {
	...
	rocketjumper = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_WEAPONJUMPING, 0, 1 );
	if ( rocketjumper < 0.5 ) {
		return qfalse;
	}
	return qtrue;
...
int BotWantsToCamp( bot_state_t *bs ) {
	...
	//if camped recently
	if ( bs->camp_time > sys->AAS_Time() - 60 + 300 * ( 1 - camper ) ) {
		return qfalse;
	}
	//
	if ( random() > camper ) {
...
//		&& (bs->inventory[INVENTORY_BFG10K] <= 0 || bs->inventory[INVENTORY_BFGAMMO] < 10)
	...
	for ( cs = sys->BotGetNextCampSpotGoal( 0, &goal ); cs; cs = sys->BotGetNextCampSpotGoal( cs, &goal ) ) {
		traveltime = sys->AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, goal.areanum, TFL_DEFAULT );
...
void BotDontAvoid( bot_state_t *bs, char *itemname ) {
	...
	num = sys->BotGetLevelItemGoal( -1, itemname, &goal );
	while ( num >= 0 ) {
		sys->BotRemoveFromAvoidGoals( bs->gs, goal.number );
		num = sys->BotGetLevelItemGoal( num, itemname, &goal );
	}
}
...
void BotRoamGoal( bot_state_t *bs, vec3_t goal ) {
	...
	for ( n = 0; n < 10; n++ ) {
		...
		if ( len > 200 ) {
			...
			if ( !trace.startsolid ) {
				trace.endpos[2]++;
				pc = sys->PointContents( trace.endpos,bs->entitynum );
...
bot_moveresult_t BotAttackMove( bot_state_t *bs, int tfl ) {
	...
	if ( bs->attackchase_time > sys->AAS_Time() ) {
		...
		sys->BotMoveToGoal( &moveresult, bs->ms, &goal, tfl );
		return moveresult;
	}
	//
	memset( &moveresult, 0, sizeof( bot_moveresult_t ) );
	//
	attack_skill = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_ATTACK_SKILL, 0, 1 );
	jumper = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_JUMPER, 0, 1 );
	croucher = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_CROUCHER, 0, 1 );
	...
	movetype = MOVE_WALK;
	//
	if ( bs->attackcrouch_time < sys->AAS_Time() - 1 ) {
		if ( random() < jumper ) {
			movetype = MOVE_JUMP;
		}
		//wait at least one second before crouching again
		else if ( bs->attackcrouch_time < sys->AAS_Time() - 1 && random() < croucher ) {
			bs->attackcrouch_time = sys->AAS_Time() + croucher * 5;
		}
	}
	if ( bs->attackcrouch_time > sys->AAS_Time() ) {
		movetype = MOVE_CROUCH;
	}
	//if the bot should jump
	if ( movetype == MOVE_JUMP ) {
		//if jumped last frame
		if ( bs->attackjump_time > sys->AAS_Time() ) {
			movetype = MOVE_WALK;
		} else {
			bs->attackjump_time = sys->AAS_Time() + 1;
		}
	}
	...
	if ( attack_skill <= 0.4 ) {
		...
		if ( dist > attack_dist + attack_range ) {
			if ( sys->BotMoveInDirection( bs->ms, forward, 400, movetype ) ) {
				return moveresult;
			}
		}
		if ( dist < attack_dist - attack_range ) {
			if ( sys->BotMoveInDirection( bs->ms, backward, 400, movetype ) ) {
				return moveresult;
			}
		}
		return moveresult;
	...
	for ( i = 0; i < 2; i++ ) {
		...
		if ( sys->BotMoveInDirection( bs->ms, sideward, 400, movetype ) ) {
			return moveresult;
		}
...
int BotSameTeam( bot_state_t *bs, int entnum ) {
	...
	if ( gametype == GT_TEAM || gametype == GT_CTF ) {
		sys->GetConfigstring( CS_PLAYERS + bs->client, info1, sizeof( info1 ) );
		sys->GetConfigstring( CS_PLAYERS + entnum, info2, sizeof( info2 ) );
		//
...
float BotEntityVisible( int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent ) {
	...
	for ( i = 0; i < 3; i++ ) {
		...
		if ( sys->AAS_PointContents( middle ) & ( CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER ) ) {
		...
		if ( trace.fraction >= 1 || trace.ent == hitent ) {
			...
			otherinfog = ( sys->AAS_PointContents( middle ) & CONTENTS_FOG );
			if ( infog && otherinfog ) {
...
int BotFindEnemy( bot_state_t *bs, int curenemy ) {
	...
	alertness = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_ALERTNESS, 0, 1 );
	easyfragger = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_EASY_FRAGGER, 0, 1 );
	...
	for ( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		//
		if ( lastteleport_time > sys->AAS_Time() - 3 ) {
		...
		if ( curenemy >= 0 ) {
			bs->enemysight_time = sys->AAS_Time() - 2;
		} else { bs->enemysight_time = sys->AAS_Time();}
...
void BotAimAtEnemy( bot_state_t *bs ) {
	...
	//
	aim_skill = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_SKILL, 0, 1 );
	aim_accuracy = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_ACCURACY, 0, 1 );
	//
	if ( aim_skill > 0.95 ) {
		//don't aim too early
		reactiontime = 0.5 * sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1 );
		if ( bs->enemysight_time > sys->AAS_Time() - reactiontime ) {
			return;
		}
		if ( bs->teleport_time > sys->AAS_Time() - reactiontime ) {
			return;
		}
	}

	//get the weapon information
	sys->BotGetWeaponInfo( bs->ws, bs->weaponnum, &wi );
...
//		aim_accuracy = sys->Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0, 1);
//	}
//	if (wi.number == WP_SHOTGUN) {
//		aim_accuracy = sys->Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_SHOTGUN, 0, 1);
//	}
	if ( wi.number == WP_GRENADE_LAUNCHER ) {
		aim_accuracy = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_ACCURACY_GRENADELAUNCHER, 0, 1 );
		aim_skill = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_SKILL_GRENADELAUNCHER, 0, 1 );
	}
	if ( wi.number == WP_FLAMETHROWER ) {
		aim_accuracy = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_ACCURACY_LIGHTNING, 0, 1 );
	}
//	if (wi.number == WP_RAILGUN) {
//		aim_accuracy = sys->Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, 0, 1);
//	}
	if ( wi.number == WP_SILENCER ) {
		aim_accuracy = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_ACCURACY_SP5, 0, 1 );
		aim_skill = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_AIM_SKILL_SP5, 0, 1 );
	}
//	if (wi.number == WP_BFG) {
//		aim_accuracy = sys->Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_BFG10K, 0, 1);
//		aim_skill = sys->Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_BFG10K, 0, 1);
//	}
	//
	if ( aim_accuracy <= 0 ) {
		aim_accuracy = 0.0001;
	...
	if ( bs->enemyposition_time < sys->AAS_Time() ) {
		//
		bs->enemyposition_time = sys->AAS_Time() + 0.5;
		...
		if ( wi.speed ) {
			...
			if ( !( dist > 100 && VectorLength( dir ) < 32 ) ) {
				...
				if ( aim_skill > 0.8 &&
					...
					sys->AAS_PredictClientMovement( &move, bs->enemy, origin,
													PRESENCE_CROUCH, qfalse,
													dir, cmdmove, 0,
													dist * 10 / wi.speed, 0.1, 0, 0, qfalse );
					VectorCopy( move.endpos, bestorigin );
					//BotAI_Print(PRT_MESSAGE, "%1.1f predicted speed = %f, frames = %f\n", sys->AAS_Time(), VectorLength(dir), dist * 10 / wi.speed);
				}
...
//			if (wi.number == WP_BFG ||	//----(SA)	removing old weapon references
			if ( wi.number == WP_GRENADE_LAUNCHER ) {
				...
				VectorSet( goal.maxs, 8, 8, 8 );
				//
				if ( sys->BotPredictVisiblePosition( bs->lastenemyorigin, bs->lastenemyareanum, &goal, TFL_DEFAULT, target ) ) {
...
//		wi.number == WP_RAILGUN) {
	...
	if ( aim_accuracy > 0.9 && bs->enemysight_time < sys->AAS_Time() - 1 ) {
...
void BotCheckAttack( bot_state_t *bs ) {
	...
	//
	reactiontime = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_REACTIONTIME, 0, 1 );
	if ( bs->enemysight_time > sys->AAS_Time() - reactiontime ) {
		return;
	}
	if ( bs->teleport_time > sys->AAS_Time() - reactiontime ) {
	...
	//if changing weapons
	if ( bs->weaponchange_time > sys->AAS_Time() - 0.1 ) {
		return;
	}
	//check fire throttle characteristic
	if ( bs->firethrottlewait_time > sys->AAS_Time() ) {
		return;
	}
	firethrottle = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_FIRETHROTTLE, 0, 1 );
	if ( bs->firethrottleshoot_time < sys->AAS_Time() ) {
		if ( random() > firethrottle ) {
			bs->firethrottlewait_time = sys->AAS_Time() + firethrottle;
			bs->firethrottleshoot_time = 0;
		} else {
			bs->firethrottleshoot_time = sys->AAS_Time() + 1 - firethrottle;
	...
	if ( trace.ent != bs->enemy ) {
		...
		if ( wi.proj.damagetype & DAMAGETYPE_RADIAL ) {
			if ( trace.fraction * 1000 < wi.proj.radius ) {
				...
				if ( points > 0 ) {
//					selfpreservation = sys->Characteristic_BFloat(bs->character, CHARACTERISTIC_SELFPRESERVATION, 0, 1);
...
int BotEntityToActivate( int entitynum ) {
	...
	for ( ent = sys->AAS_NextBSPEntity( 0 ); ent; ent = sys->AAS_NextBSPEntity( ent ) ) {
		if ( !sys->AAS_ValueForBSPEpairKey( ent, "model", tmpmodel, sizeof( tmpmodel ) ) ) {
			continue;
		}
	...
	sys->AAS_ValueForBSPEpairKey( ent, "classname", classname, sizeof( classname ) );
	if ( !classname ) {
	...
	if ( !strcmp( classname, "func_door" ) ) {
		if ( sys->AAS_FloatForBSPEpairKey( ent, "health", &health ) ) {
	...
	if ( !sys->AAS_ValueForBSPEpairKey( ent, "targetname", targetname[0], sizeof( targetname[0] ) ) ) {
#ifdef OBSTACLEDEBUG
		BotAI_Print( PRT_ERROR, "BotEntityToActivate: entity with model \"%s\" has no targetname\n", model );
#endif //OBSTACLEDEBUG
		return 0;
	}
	cur_entities[0] = sys->AAS_NextBSPEntity( 0 );
	for ( i = 0; i >= 0 && i < 10; ) {
		for ( ent = cur_entities[i]; ent; ent = sys->AAS_NextBSPEntity( ent ) ) {
			if ( !sys->AAS_ValueForBSPEpairKey( ent, "target", target, sizeof( target ) ) ) {
				continue;
			}
			if ( !strcmp( targetname[i], target ) ) {
				cur_entities[i] = sys->AAS_NextBSPEntity( ent );
				break;
			}
		}
		if ( !ent ) {
		...
		if ( !sys->AAS_ValueForBSPEpairKey( ent, "classname", classname, sizeof( classname ) ) ) {
...
//		BotAI_Print(PRT_MESSAGE, "%s: I've got no brain cells for activating entities\n", netname);
	   ...
	   if (!strcmp(classname, "func_door"))
	   {
		...
		if ( !strcmp( classname, "func_button" ) ) {
			//create a bot goal towards the button
			sys->AAS_ValueForBSPEpairKey( ent, "model", model, sizeof( model ) );
			...
			//get the lip of the button
			sys->AAS_FloatForBSPEpairKey( ent, "lip", &lip );
			if ( !lip ) {
				lip = 4;
			}
			//get the move direction from the angle
			sys->AAS_FloatForBSPEpairKey( ent, "angle", &angle );
			...
			dist *= 0.5;
			//
			sys->AAS_FloatForBSPEpairKey( ent, "health", &health );
			//if the button is shootable
			if ( health ) {
				//calculate the goal origin
				VectorMA( origin, -dist, movedir, goalorigin );
				...
				moveresult->flags |= MOVERESULT_MOVEMENTVIEW;
				//select the blaster
				sys->EA_SelectWeapon( bs->client, WEAPONINDEX_MACHINEGUN );
				//shoot
				sys->EA_Attack( bs->client );
				return;
			} //end if
			else
			{
				...
				numareas = sys->AAS_TraceAreas( start, end, areas, NULL, 10 );
				//
				for ( i = 0; i < numareas; i++ ) {
					if ( sys->AAS_AreaReachability( areas[i] ) ) {
						break;
					}
				}
				if ( i < numareas ) {
					//
#ifdef OBSTACLEDEBUG
					if ( bs->activatemessage_time < sys->AAS_Time() ) {
...
//	if (bsptrace.fraction >= 1) movetype = MOVE_CROUCH;
	...
	if ( movetype != MOVE_CROUCH || !sys->BotMoveInDirection( bs->ms, hordir, 400, movetype ) ) {
		//perform the movement
		if ( !sys->BotMoveInDirection( bs->ms, sideward, 400, movetype ) ) {
			...
			sys->BotMoveInDirection( bs->ms, sideward, 400, movetype );
		}
	}
...
void BotCheckConsoleMessages( bot_state_t *bs ) {
	...
	while ( ( handle = sys->BotNextConsoleMessage( bs->cs, &m ) ) != 0 ) {
		...
		sys->BotReplaceSynonyms( m.message, context );
		//if there's no match
		if ( !BotMatchMessage( bs, m.message ) ) {
			//if it is a chat message
			if ( m.type == CMS_CHAT && !bot_nochat.integer ) {
				//
				if ( !sys->BotFindMatch( m.message, &match, MTCONTEXT_REPLYCHAT ) ) {
					sys->BotRemoveConsoleMessage( bs->cs, handle );
					continue;
				}
				...
				//
				sys->BotMatchVariable( &match, NETNAME, netname, sizeof( netname ) );
				sys->BotMatchVariable( &match, MESSAGE, message, sizeof( message ) );
				...
				//unify the message
				sys->UnifyWhiteSpaces( message );
				//
				sys->Cvar_Update( &bot_testrchat );
				if ( bot_testrchat.integer ) {
					//
					sys->BotLibVarSet( "bot_testrchat", "1" );
					//if bot replies with a chat message
					if ( sys->BotReplyChat( bs->cs, message, context, CONTEXT_REPLY,
											NULL, NULL,
											NULL, NULL,
											NULL, NULL,
											botname, netname ) ) {
				...
				//if at a valid chat position and not chatting already
					...
					chat_reply = sys->Characteristic_BFloat( bs->character, CHARACTERISTIC_CHAT_REPLY, 0, 1 );
					if ( random() < 1.5 / ( NumBots() + 1 ) && random() < chat_reply ) {
						...
						if ( sys->BotReplyChat( bs->cs, message, context, CONTEXT_REPLY,
							...
							sys->BotRemoveConsoleMessage( bs->cs, handle );
							bs->stand_time = sys->AAS_Time() + BotChatTime( bs );
							AIEnter_Stand( bs );
...
void BotCheckEvents( bot_state_t *bs, entityState_t *state ) {
	...
	case EV_OBITUARY:
	{
		...
		//else if this client was killed by the bot
			...
			bs->killedenemy_time = sys->AAS_Time();
			//
			bs->num_kills++;
	...
	case EV_GLOBAL_SOUND:
	{
		...
		sys->GetConfigstring( CS_SOUNDS + state->eventParm, buf, sizeof( buf ) );
	...
	case EV_PLAYER_TELEPORT_IN:
	{
		...
		lastteleport_time = sys->AAS_Time();
		break;
	}
...
void BotCheckAir( bot_state_t *bs ) {
	if ( bs->inventory[INVENTORY_ENVIRONMENTSUIT] <= 0 ) {
		if ( sys->AAS_PointContents( bs->eye ) & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) {
			return;
		}
	}
	bs->lastair_time = sys->AAS_Time();
}
...
void BotDeathmatchAI( bot_state_t *bs, float thinktime ) {
	...
	if ( bs->setupcount > 0 ) {
		...
		sys->Characteristic_String( bs->character, CHARACTERISTIC_GENDER, gender, sizeof( gender ) );
		//set the bot gender
		sys->GetUserinfo( bs->client, userinfo, sizeof( userinfo ) );
		Info_SetValueForKey( userinfo, "sex", gender );
		sys->SetUserinfo( bs->client, userinfo );
		//set the team
		if ( g_gametype.integer != GT_TOURNAMENT ) {
			Com_sprintf( buf, sizeof( buf ), "team %s", bs->settings.team );
			sys->EA_Command( bs->client, buf );
		}
		//set the chat gender
		if ( gender[0] == 'm' ) {
		...
		} else { sys->BotSetChatGender( bs->cs, CHAT_GENDERLESS );}
		//set the chat name
		ClientName( bs->client, name, sizeof( name ) );
		sys->BotSetChatName( bs->cs, name );
		//
	...
	if ( !bs->entergamechat && bs->entergame_time > sys->AAS_Time() - 8 ) {
	...
	if ( i >= MAX_NODESWITCHES ) {
		sys->BotDumpGoalStack( bs->gs );
		sys->BotDumpAvoidGoals( bs->gs );
		BotDumpNodeSwitches( bs );
		ClientName( bs->client, name, sizeof( name ) );
		BotAI_Print( PRT_ERROR, "%s at %1.1f switched more than %d AI nodes\n", name, sys->AAS_Time(), MAX_NODESWITCHES );
	}
	//
	bs->lastframe_health = bs->inventory[INVENTORY_HEALTH];
	bs->lasthitcount = bs->cur_ps.persistant[PERS_HITS];
...
void BotSetupDeathmatchAI( void ) {
	...
	gametype = sys->Cvar_VariableIntegerValue( "g_gametype" );

	// Rafael gameskill
	gameskill = sys->Cvar_VariableIntegerValue( "g_gameskill" );
	// done

	sys->Cvar_Register( &bot_rocketjump, "bot_rocketjump", "1", 0 );
	sys->Cvar_Register( &bot_grapple, "bot_grapple", "0", 0 );
	sys->Cvar_Register( &bot_fastchat, "bot_fastchat", "0", 0 );
	sys->Cvar_Register( &bot_nochat, "bot_nochat", "0", 0 );
	sys->Cvar_Register( &bot_testrchat, "bot_testrchat", "0", 0 );
	//
	if ( gametype == GT_CTF ) {
		if ( sys->BotGetLevelItemGoal( -1, "Red Flag", &ctf_redflag ) < 0 ) {
			BotAI_Print( PRT_WARNING, "CTF without Red Flag\n" );
		}
		if ( sys->BotGetLevelItemGoal( -1, "Blue Flag", &ctf_blueflag ) < 0 ) {
	...
	max_bspmodelindex = 0;
	for ( ent = sys->AAS_NextBSPEntity( 0 ); ent; ent = sys->AAS_NextBSPEntity( ent ) ) {
		if ( !sys->AAS_ValueForBSPEpairKey( ent, "model", model, sizeof( model ) ) ) {
			continue;
		}
		if ( model[0] == '*' ) {
