...
// (SA) i don't know about these, but we don't have models for 'em
	...
	weaponInfo->weaponModel[W_TP_MODEL] = sys->R_RegisterModel( item->world_model[W_TP_MODEL] );
	weaponInfo->weaponModel[W_FP_MODEL] = sys->R_RegisterModel( item->world_model[W_FP_MODEL] );
	weaponInfo->weaponModel[W_SKTP_MODEL] = sys->R_RegisterModel( item->world_model[W_SKTP_MODEL] );
...
//----(SA)	end
	...
	sys->R_ModelBounds( weaponInfo->weaponModel[W_TP_MODEL], mins, maxs );

	for ( i = 0 ; i < 3 ; i++ ) {
		weaponInfo->weaponMidpoint[i] = mins[i] + 0.5 * ( maxs[i] - mins[i] );
	}

	weaponInfo->weaponIcon[0] = sys->R_RegisterShader( item->icon );
	weaponInfo->weaponIcon[1] = sys->R_RegisterShader( va( "%s_select", item->icon ) );    // get the 'selected' icon as well

	// JOSEPH 4-17-00
	weaponInfo->ammoIcon = sys->R_RegisterShader( item->ammoicon );
	// END JOSEPH
	...
	if ( ammo->classname && ammo->world_model[0] ) {
		weaponInfo->ammoModel = sys->R_RegisterModel( ammo->world_model[0] );
	}
	...
	for ( i = W_TP_MODEL; i < W_NUM_TYPES; i++ )
	{
		...
		weaponInfo->flashModel[i] = sys->R_RegisterModel( path );
		...
		for ( j = 0; j < W_MAX_PARTS; j++ ) {
			...
			weaponInfo->wpPartModels[i][j] = sys->R_RegisterModel( path );
		}
		...
		if ( i == W_FP_MODEL ) {
			...
			weaponInfo->wpPartModels[i][W_PART_7] = sys->R_RegisterModel( path );
		}
	}
	...
	if ( weaponNum == WP_MAUSER || weaponNum == WP_GARAND ) {
		...
		weaponInfo->modModel[0] = sys->R_RegisterModel( path );
	}
	...
	weaponInfo->handsModel = sys->R_RegisterModel( path );

	if ( !weaponInfo->handsModel ) {
		weaponInfo->handsModel = sys->R_RegisterModel( "models/weapons2/shotgun/shotgun_hand.md3" );
	}
...
//----(SA)	weapon pickup 'stand'
	...
	weaponInfo->standModel = sys->R_RegisterModel( path );
//----(SA)	end
	...
	case WP_AKIMBO: //----(SA)	added
		...
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/colt/coltf1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mp40/mp40e1.wav" ); // use same as mp40
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;

		// unique
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/colt/colt_reload2.wav" );
		break;
	...
	case WP_COLT:
		...
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/colt/coltf1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mp40/mp40e1.wav" ); // use same as mp40
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/colt/colt_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;


	case WP_KNIFE:
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/knife/knife_slash1.wav" );
		weaponInfo->flashSound[1] = sys->S_RegisterSound( "sound/weapons/knife/knife_slash2.wav" );
		break;

	case WP_LUGER:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );

		weaponInfo->switchSound[0] = sys->S_RegisterSound( "sound/weapons/luger/silencerremove.wav" );   //----(SA)	added

		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/luger/lugerf1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mp40/mp40e1.wav" ); // use same as mp40
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/luger/luger_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	...
	case WP_SILENCER:   // luger mod
		...
		weaponInfo->switchSound[0] = sys->S_RegisterSound( "sound/weapons/luger/silencerattatch.wav" );  //----(SA)	added

		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/luger/silencerf1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/luger/luger_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	...
	case WP_MAUSER:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/mauser/mauserf1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mauser/mausere1.wav" );
		weaponInfo->lastShotSound[0] = sys->S_RegisterSound( "sound/weapons/mauser/mauserf1_last.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/mauser/mauser_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	case WP_SNIPERRIFLE:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/mauser/sniperf1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mauser/mausere1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/mauser/sniper_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;

	case WP_GARAND:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/garand/garandf1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/garand/garand_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	case WP_SNOOPERSCOPE:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/garand/snooperf1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/garand/snooper_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;

	case WP_THOMPSON:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/thompson/thompson.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mp40/mp40e1.wav" ); // use same as mp40
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/thompson/thompson_reload.wav" );
		weaponInfo->overheatSound = sys->S_RegisterSound( "sound/weapons/thompson/thompson_overheat.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;

	case WP_MP40:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/mp40/mp40f1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/mp40/mp40e1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/mp40/mp40_reload.wav" );
		weaponInfo->overheatSound = sys->S_RegisterSound( "sound/weapons/mp40/mp40_overheat.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;

	case WP_STEN:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/sten/stenf1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/sten/sten_reload.wav" );
		weaponInfo->overheatSound = sys->S_RegisterSound( "sound/weapons/sten/sten_overheat.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	...
	case WP_FG42SCOPE:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/fg42/fg42f1.wav" );
		weaponInfo->flashEchoSound[0] = sys->S_RegisterSound( "sound/weapons/fg42/fg42e1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/fg42/fg42_reload.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	...
	case WP_PANZERFAUST:
		...
		weaponInfo->missileModel        = sys->R_RegisterModel( "models/ammo/rocket/rocket.md3" );
		weaponInfo->missileSound        = sys->S_RegisterSound( "sound/weapons/rocket/rockfly.wav" );
		...
		weaponInfo->flashSound[0]       = sys->S_RegisterSound( "sound/weapons/rocket/rocklf1a.wav" );
		weaponInfo->reloadSound         = sys->S_RegisterSound( "sound/weapons/rocket/rocklf_reload.wav" );
		cgs.media.rocketExplosionShader = sys->R_RegisterShader( "rocketExplosion" );
		break;

	case WP_MORTAR:
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/mortar/mortarf1.wav" );
...
// JPW NERVE
	case WP_GRENADE_SMOKE:
		weaponInfo->missileModel = sys->R_RegisterModel( "models/weapons2/grenade/pineapple.md3" );
...
// dhm
	...
	case WP_GRENADE_PINEAPPLE:
		if ( weaponNum == WP_GRENADE_LAUNCHER ) {
			weaponInfo->missileModel = sys->R_RegisterModel( "models/ammo/grenade1.md3" );
		} else {
			weaponInfo->missileModel = sys->R_RegisterModel( "models/weapons2/grenade/pineapple.md3" );
		}
		weaponInfo->missileTrailFunc = CG_GrenadeTrail;
...
//		weaponInfo->wiTrailTime = 2000;
		...
		weaponInfo->trailRadius = 32;
		MAKERGB( weaponInfo->flashDlightColor, 1, 0.7, 0.5 );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/grenade/grenlf1a.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/grenade/grenlf_reload.wav" );
		cgs.media.grenadeExplosionShader = sys->R_RegisterShader( "grenadeExplosion" );
		break;

	case WP_DYNAMITE:
		weaponInfo->missileModel = sys->R_RegisterModel( "models/ammo/dynamite.md3" );
//		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/grenade/grenlf1a.wav" );
//		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/grenade/grenlf_reload.wav" );
		cgs.media.grenadeExplosionShader = sys->R_RegisterShader( "grenadeExplosion" );
		break;
	...
	case WP_VENOM:
		MAKERGB( weaponInfo->flashDlightColor, 1.0, 0.6, 0.23 );
		weaponInfo->spinupSound = sys->S_RegisterSound( "sound/weapons/venom/venomsu1.wav" );    //----(SA)	added
		weaponInfo->spindownSound = sys->S_RegisterSound( "sound/weapons/venom/venomsd1.wav" );  //----(SA)	added
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/venom/venomf1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/venom/venom_reload.wav" );
		weaponInfo->overheatSound = sys->S_RegisterSound( "sound/weapons/venom/venom_overheat.wav" );
		weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
		break;
	...
	case WP_TESLA:
		...
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/tesla/teslaf1.wav" );
		weaponInfo->reloadSound = sys->S_RegisterSound( "sound/weapons/tesla/tesla_reload.wav" );
		weaponInfo->overheatSound = sys->S_RegisterSound( "sound/weapons/tesla/tesla_overheat.wav" );
		break;


	case WP_GAUNTLET:
		MAKERGB( weaponInfo->flashDlightColor, 0.6, 0.6, 1 );
		//weaponInfo->firingSound = sys->S_RegisterSound( "sound/weapons/melee/fstrun.wav" );
		weaponInfo->flashSound[0] = sys->S_RegisterSound( "sound/weapons/melee/fstatck.wav" );
		break;
...
//----(SA)		the first thing it does is to check if it's registered.
	...
	for ( i = 0; i < MAX_ITEM_MODELS; i++ )
		itemInfo->models[i] = sys->R_RegisterModel( item->world_model[i] );


	itemInfo->icons[0] = sys->R_RegisterShader( item->icon );
	if ( item->giType == IT_HOLDABLE ) {
		...
		for ( i = 1; i < MAX_ITEM_ICONS; i++ )
			itemInfo->icons[i] = sys->R_RegisterShader( va( "%s%i", item->icon, i + 1 ) );
	}
	...
	wolfkickModel = sys->R_RegisterModel( "models/weapons2/foot/v_wolfoot_10f.md3" );
	hWeaponSnd = sys->S_RegisterSound( "sound/weapons/mg42/37mm.wav" );

	hflakWeaponSnd = sys->S_RegisterSound( "sound/weapons/flak/flak.wav" );

	notebookModel = sys->R_RegisterModel( "models/mapobjects/book/book.md3" );

	propellerModel = sys->R_RegisterModel( "models/mapobjects/vehicles/m109_prop.md3" );
...
// JPW NERVE had to put this somewhere, this seems OK
	if ( cg_gameType.integer != GT_WOLF ) {
		...
		sys->R_RegisterModel( "models/mapobjects/vehicles/m109.md3" );
...
//		if ( item->world_model[W_FP_MODEL] ) {
//			itemInfo->models[W_FP_MODEL] = sys->R_RegisterModel( item->world_model[W_FP_MODEL] );
//		}
//	}
}
	...
	if ( cent->pe.barrelSpinning == !firing ) {
		...
		if ( cent->pe.barrelSpinning ) {
			if ( cg_weapons[WP_VENOM].spinupSound ) {
				sys->S_StartSoundEx( NULL, cent->currentState.number, CHAN_WEAPON, cg_weapons[WP_VENOM].spinupSound, SND_OKTOCUT );
			}
		} else {
			if ( cg_weapons[WP_VENOM].spindownSound ) {
				sys->S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, cg_weapons[WP_VENOM].spindownSound );
			}
		}
...
*/                                                                                                                              //
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		if ( VectorDistance( tagPos, cent->pe.teslaEndPoints[i] ) <= maxDist ) {
			...
			if ( cent->pe.lightningSoundTime < cg.time - 200 ) {
				...
				//sys->S_StartSound( cent->pe.teslaEndPoints[i], ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.lightningSounds[rand()%3] );
	...
	if ( cg.time % 3 ) {  // break it up a bit
		...
		sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.teslaLoopSound, 255 );
	}
	...
	if ( ( cg.time / 50 ) % ( 4 + ( cg.time % 4 ) ) == 0 ) {
		// alt light
		sys->R_AddLightToScene( tr.endpos, 256 + 600 * tr.fraction, 0.2, 0.6, 1, 1 );
	} else if ( ( cg.time / 50 ) % ( 4 + ( cg.time % 4 ) ) == 1 ) {
		// no light
		//sys->R_AddLightToScene( tr.endpos, 128 + 500*tr.fraction, 1, 1, 1, 10 );
	} else {
		// blue light
		sys->R_AddLightToScene( tr.endpos, 256 + 600 * tr.fraction, 0.2, 0.6, 1, 0 );
	}
...
void CG_AddPlayerWeapon( refEntity_t *parent, playerState_t *ps, centity_t *cent ) {
	...
	if ( !ps ) {
		...
		if ( ( cent->currentState.eFlags & EF_FIRING ) && weapon->firingSound ) {
			...
			sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->firingSound, 255 );
			cent->pe.lightningFiring = qtrue;
		} else if ( weapon->readySound ) {
			sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->readySound, 255 );
		}
	}
...
//					CG_ParticleImpactSmokePuff (cgs.media.smokeParticleShader, flash.origin);
	...
	if ( flash.hModel ) {
		if ( weaponNum != WP_FLAMETHROWER && weaponNum != WP_TESLA ) {    //Ridah, hide the flash also for now
			...
			if ( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME ) {
//			if (firing) {	// Ridah
				sys->R_AddRefEntityToScene( &flash );
			}
		}
	}
	...
	if ( ps || cg.renderingThirdPerson || !isPlayer ) {
		...
		if ( firing ) {
			...
			if ( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] ) {
				sys->R_AddLightToScene( flash.origin, 200 + ( rand() & 31 ), weapon->flashDlightColor[0],
...
//	CG_Printf("frame: %d\n", frame);
	...
	sys->R_AddRefEntityToScene( &wolfkick );
...
//	weap = &cg_weapons[ ent->weapon ];
	...
	sys->S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_WEAPON, switchsound );
}
...
//	sys->S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, hWeaponSnd );
	...
	sys->R_AddLightToScene( point, 200 + ( rand() & 31 ),1.0, 0.6, 0.23, 0 );
...
void CG_FLAKEFX( centity_t *cent, int whichgun ) {
	...
	sys->S_StartSound( NULL, ent->number, CHAN_WEAPON, hflakWeaponSnd );
}
...
void CG_MortarEFX( centity_t *cent ) {
	...
	if ( cent->currentState.density & 2 ) {
		// light
		sys->R_AddLightToScene( cent->currentState.origin, 200 + ( rand() & 31 ), 1.0, 1.0, 1.0, 0 );
...
void CG_FireWeapon( centity_t *cent ) {
	...
	if ( ( cent->currentState.clientNum == cg.snap->ps.clientNum && cg.snap->ps.persistant[PERS_HWEAPON_USE] ) ||
		...
		sys->S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, hWeaponSnd );
		//sys->S_StartSound( NULL, ent->number, CHAN_WEAPON, hWeaponSnd );
		if ( cg_brassTime.integer > 0 ) {
	...
	if ( cent->currentState.powerups & ( 1 << PW_QUAD ) ) {
		sys->S_StartSound( NULL, cent->currentState.number, CHAN_ITEM, cgs.media.quadSound );
	}
	...
	if ( c > 0 ) {
		c = rand() % c;
		if ( firesound[c] ) {
			sys->S_StartSound( NULL, ent->number, CHAN_WEAPON, firesound[c] );
			...
			if ( fireEchosound && fireEchosound[c] ) { // check for echo
				...
				if ( gdist > 512 && gdist < 4096 ) {   // temp dist.  TODO: use numbers that are weapon specific
					...
					sys->S_StartSoundEx( gorg, ent->number, CHAN_WEAPON, fireEchosound[c], SND_NOCUT );
				}
			}
		}
	}
...
void CG_MissileHitPlayer( centity_t *cent, int weapon, vec3_t origin, vec3_t dir, int entityNum ) {
	...
	case WP_KNIFE:
		...
		if ( cgs.media.sfx_knifehit[i] ) {
			sys->S_StartSound( origin, cent->currentState.number, CHAN_WEAPON, cgs.media.sfx_knifehit[i] );
		}
...
void CG_VenomFire( entityState_t *es, qboolean fullmode ) {
	...
	if ( cgs.glconfig.hardwareType != GLHW_RAGEPRO ) {
		...
		contents = sys->CM_PointContents( es->pos.trBase, 0 );
...
void CG_DrawTracer( vec3_t start, vec3_t finish ) {
	...
	sys->R_AddPolyToScene( cgs.media.tracerShader, 4, verts );
}
...
void CG_Tracer( vec3_t source, vec3_t dest, int sparks ) {
	...
	// sys->S_StartSound( midpoint, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.tracerSound );
...
void CG_Bullet( vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum, qboolean wolfkick, int otherEntNum2 ) {
	...
	if ( sourceEntityNum >= 0 && cg_tracerChance.value > 0 ) {
		if ( CG_CalcMuzzlePoint( sourceEntityNum, start ) ) {
			sourceContentType = sys->CM_PointContents( start, 0 );
			destContentType = sys->CM_PointContents( end, 0 );
			...
			// bubble trail from water into air
				...
				sys->CM_BoxTrace( &trace, end, start, NULL, NULL, 0, CONTENTS_WATER );
...
//							0.5, 80, 16, 0.125, "water_splash" );	// rand scale
				...
				if ( Distance( cg.snap->ps.origin, end ) < 1024 ) {
					sys->CM_BoxTrace( &trace, start, end, NULL, NULL, 0, CONTENTS_WATER );
	...
	if ( flesh ) {
		...
		if (cent->currentState.aiChar == AICHAR_ZOMBIE) {
			...
			VectorNormalize( dir );
			// upper
			sys->RB_ZombieFXAddNewHit( cent->currentState.number, end, dir );
			// lower
			sys->RB_ZombieFXAddNewHit( cent->currentState.number | (1<<30), end, dir );
			return;
		}
		*/
		...
		if ( lastBloodSpat > cg.time || lastBloodSpat < cg.time - 500 ) {
			if ( CG_CalcMuzzlePoint( sourceEntityNum, start ) ) {
				...
				sys->CM_BoxTrace( &trace, end, trend, NULL, NULL, 0, MASK_SHOT & ~CONTENTS_BODY );
...
//		}
		...
		if ( CG_CalcMuzzlePoint( sourceEntityNum, start )
			...
			sys->CM_BoxTrace( &trace, start2, dir, NULL, NULL, 0, MASK_SHOT );
...
//				CG_MissileHitWall( fromweap, 0, end, normal, trace.surfaceFlags );	// sparks	//	(SA) modified to send missilehitwall surface parameters
			...
			if ( !( sourceContentType & CONTENTS_WATER ) && ( destContentType & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) ) {   // only when shooting /into/ water
				sys->CM_BoxTrace( &trace, start, end, NULL, NULL, 0, MASK_WATER );
...
void CG_ClientDamage( int entnum, int enemynum, int id ) {
	...
	// -NERVE - SMF
		...
		sys->SendClientCommand( va( "cld %i %i %i", entnum, enemynum, id ) );
	}
}
