...
64-bit port notes:
- Removed the old SWAP_DRAW_SURF macro that assumed drawSurf_t was exactly 8 bytes.
- The old code reinterpreted drawSurf_t as int[2], which is not safe on 64-bit.
- We now swap whole drawSurf_t objects directly.
=================
*/

static ID_INLINE void SwapDrawSurf(drawSurf_t* a, drawSurf_t* b) {
	drawSurf_t temp = *a;
	*a = *b;
	*b = temp;
}
...
void qsortFast(
	...
	/* This sorter is specialized for drawSurf_t. */
	if (width != sizeof(drawSurf_t)) {
		ri.Error(ERR_DROP, "qsortFast: width != sizeof(drawSurf_t)");
		return;
	}
...
recurse:
	...
	if (size <= CUTOFF) {
		...
		/* First we pick a partitioning element. The efficiency of the
		   ...
		   the first one produces bad performance if the array is already
		...
		/* We now wish to partition the array into three pieces, one
		   ...
		   equal to the partition element, and one of elements >= to it. */
