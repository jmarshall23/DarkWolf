...
void CG_PositionEntityOnTag( refEntity_t *entity, const refEntity_t *parent,
	...
	// lerp the tag
	sys->R_LerpTag( &lerped, parent, tagName, startIndex );
...
//AxisClear( entity->axis );
	// lerp the tag
	sys->R_LerpTag( &lerped, parent, tagName, 0 );
...
void CG_AddLightstyle( centity_t *cent ) {
	...
	sys->R_AddLightToScene( cent->lerpOrigin, lightval, (float)r / 255.0f, (float)g / 255.0f, (float)b / 255.0f, 0 ); // overdraw forced to 0 for now
}
...
static void CG_EntityEffects( centity_t *cent ) {
	...
	if ( cent->currentState.loopSound ) {
		...
		if ( cent->currentState.eType == ET_SPEAKER ) {
			/*if(cent->currentState.density == 1) {	// NO_PVS
				sys->S_AddRealLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.gameSounds[ cent->currentState.loopSound ] );
			}
		...
		if(cent->currentState.eType == ET_SPEAKER) {
			if(cent->currentState.density == 1) {
				sys->S_StopLoopingSound(cent->currentState.number);
			}
		}
	}*/
	...
	if ( cent->currentState.constantLight ) {
		...
		} else
		{
			...
			sys->R_AddLightToScene( cent->lerpOrigin, i, (float)r / 255.0f, (float)g / 255.0f, (float)b / 255.0f, 0 );
		}
	}
	...
	if ( CG_EntOnFire( cent ) ) {
		...
		sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.flameBlowSound, (int)( 255.0 * ( 1.0 - fabs( cent->fireRiseDir[2] ) ) ) );
		// play a burning sound when not moving
		sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.flameSound, (int)( 0.3 * 255.0 * ( pow( cent->fireRiseDir[2],2 ) ) ) );
	}
...
static void CG_Speaker( centity_t *cent ) {
	...
	sys->S_StartSound( NULL, cent->currentState.number, CHAN_ITEM, cgs.gameSounds[cent->currentState.eventParm] );
...
//----(SA)	end
	...
	if ( !( cent->usehighlightOrigin ) ) {
		...
		sys->R_ModelBounds( ent.hModel, mins, maxs );           // get bounds
...
static void CG_Missile( centity_t *cent ) {
	...
	if ( weapon->missileDlight ) {
		sys->R_AddLightToScene( cent->lerpOrigin, weapon->missileDlight,
...
//----(SA)	whoops, didn't mean to check it in with the missile flare
	...
	if ( weapon->missileSound ) {
		...
		sys->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, velocity, weapon->missileSound, 255 );
	}
...
//cgs.media.ssSpiritSkullModel = sys->R_RegisterModel( "models/players/supersoldier/ssghost.md3" );
	...
	// emit a sound
	sys->S_AddLoopingSound( 0, refent.origin, vec3_origin, cgs.media.zombieSpiritLoopSound, 255 );
}
...
//	CG_Printf("dot: %f\n", dot);
	...
	if ( !behind && !toofar ) {
		...
		sys->R_AddCoronaToScene( cent->lerpOrigin, (float)r / 255.0f, (float)g / 255.0f, (float)b / 255.0f, (float)cent->currentState.density / 255.0f, cent->currentState.number, flags );
	}
}
...
//						cg_entities[trace.entityNum].pe.teslaDamagedTime = cg.time;
		...
		if ( cent->currentState.frame == 1 ) { // dead, don't move
			...
			if ( rand() % 50 == 1 ) {
				...
				sys->S_StartSound( NULL, cent->currentState.number, CHAN_AUTO, cgs.media.sparkSounds[0] );
			}
		...
		if ( !splinetarget ) {
			...
			if ( splinetarget != -1 ) {
				if ( sys->GetCameraInfo( splinetarget, time, &targetpos, &angles, &fov ) ) {
					...
					sys->StartCamera( splinetarget, cg.time );
					sys->GetCameraInfo( splinetarget, cg.time, &targetpos, &angles, &fov );
				}
			}
		}
...
//----(SA)	end
	...
	if ( cent->currentState.density == 7 || cent->currentState.density == 8 ) {
		...
		sys->R_AddRefEntityToScene( &propeller );
		...
		if ( cent->currentState.density == 8 ) {
			...
			sys->R_AddLightToScene( flash.origin, 200 + ( rand() & 31 ),1.0, 0.6, 0.23, 0 );
