...
int CG_GetOriginForTag( centity_t *cent, refEntity_t *parent, char *tagName, int startIndex, vec3_t org, vec3_t axis[3] ) {
	...
	// lerp the tag
	retval = sys->R_LerpTag( &lerped, parent, tagName, startIndex );
...
//
	if ( !( flags & SL_NOCORE ) ) {
		...
		sys->R_AddPolyToScene( cgs.media.spotLightBeamShader, 4, &coreverts[0] );
	}
...
//
	...
	sys->R_AddPolysToScene( cgs.media.spotLightBeamShader, 4, &verts[0], segs );


	// plug up the start circle
	if ( capStart ) {
		sys->R_AddPolyToScene( cgs.media.spotLightBeamShader, segs, &plugVerts[0] );
	}
...
//		sys->R_AddLightToScene(dlightLoc, 200, colorNorm[0], colorNorm[1], colorNorm[2], 0);	// ,REF_JUNIOR_DLIGHT);
//		sys->R_AddLightToScene(dlightLoc, radius*2, colorNorm[0], colorNorm[1], colorNorm[2], 0);	// ,REF_JUNIOR_DLIGHT);
		sys->R_AddLightToScene( dlightLoc, radius * 2, 0.3, 0.3, 0.3, 0 );  // ,REF_JUNIOR_DLIGHT);
	}
