...
void BotPrintTeamGoal( bot_state_t *bs ) {
	...
	t = bs->teamgoal_time - sys->AAS_Time();
	switch ( bs->ltgtype ) {
	case LTG_TEAMHELP:
	{
	...
	default:
	{
		if ( bs->ctfroam_time > sys->AAS_Time() ) {
			t = bs->ctfroam_time - sys->AAS_Time();
...
int BotGetItemTeamGoal( char *goalname, bot_goal_t *goal ) {
	...
	i = -1;
	do {
		i = sys->BotGetLevelItemGoal( i, goalname, goal );
...
float BotGetTime( bot_match_t *match ) {
	...
	if ( match->subtype & ST_TIME ) {
		//get the time string
		sys->BotMatchVariable( match, TIME, timestring, MAX_MESSAGE_SIZE );
		//match it to find out if the time is in seconds or minutes
		if ( sys->BotFindMatch( timestring, &timematch, MTCONTEXT_TIME ) ) {
			if ( timematch.type == MSG_FOREVER ) {
				t = 99999999;
			} else {
				sys->BotMatchVariable( &timematch, TIME, timestring, MAX_MESSAGE_SIZE );
...
int NumPlayersOnSameTeam( bot_state_t *bs ) {
	...
	num = 0;
	for ( i = 0; i < maxclients && i < MAX_CLIENTS; i++ ) {
		sys->GetConfigstring( CS_PLAYERS + i, buf, MAX_INFO_STRING );
		if ( strlen( buf ) ) {
...
int BotGetPatrolWaypoints( bot_state_t *bs, bot_match_t *match ) {
	...
	patrolflags = 0;
	//
	sys->BotMatchVariable( match, KEYAREA, keyarea, MAX_MESSAGE_SIZE );
	//
	while ( 1 ) {
		if ( !sys->BotFindMatch( keyarea, &keyareamatch, MTCONTEXT_PATROLKEYAREA ) ) {
			sys->EA_SayTeam( bs->client, "what do you say?" );
			BotFreeWaypoints( newpatrolpoints );
			bs->patrolpoints = NULL;
			return qfalse;
		}
		sys->BotMatchVariable( &keyareamatch, KEYAREA, keyarea, MAX_MESSAGE_SIZE );
		...
		if ( keyareamatch.subtype & ST_BACK ) {
			...
			sys->BotMatchVariable( &keyareamatch, MORE, keyarea, MAX_MESSAGE_SIZE );
		} else {
			break;
		}
	}
	...
	if ( !newpatrolpoints || !newpatrolpoints->next ) {
		sys->EA_SayTeam( bs->client, "I need more key points to patrol\n" );
		BotFreeWaypoints( newpatrolpoints );
		newpatrolpoints = NULL;
		return qfalse;
	}
...
int BotAddressedToBot( bot_state_t *bs, bot_match_t *match ) {
	...
	if ( match->subtype & ST_ADDRESSED ) {
		sys->BotMatchVariable( match, ADDRESSEE, addressedto, sizeof( addressedto ) );
		//the name of this bot
		ClientName( bs->client, botname, 128 );
		//
		while ( sys->BotFindMatch( addressedto, &addresseematch, MTCONTEXT_ADDRESSEE ) ) {
			if ( addresseematch.type == MSG_EVERYONE ) {
				return qtrue;
			} else if ( addresseematch.type == MSG_MULTIPLENAMES )     {
				sys->BotMatchVariable( &addresseematch, TEAMMATE, name, sizeof( name ) );
				if ( strlen( name ) ) {
				...
				sys->BotMatchVariable( &addresseematch, MORE, addressedto, MAX_MESSAGE_SIZE );
			} else {
				sys->BotMatchVariable( &addresseematch, TEAMMATE, name, MAX_MESSAGE_SIZE );
				if ( strlen( name ) ) {
					if ( stristr( botname, name ) ) {
						return qtrue;
					}
...
void BotMatch_HelpAccompany( bot_state_t *bs, bot_match_t *match ) {
	...
	sys->BotMatchVariable( match, TEAMMATE, teammate, sizeof( teammate ) );
	//get the client to help
	if ( sys->BotFindMatch( teammate, &teammatematch, MTCONTEXT_TEAMMATE ) &&
	...
	if ( bs->teamgoal.entitynum < 0 ) {
		...
		if ( match->subtype & ST_NEARITEM ) {
			...
			sys->BotMatchVariable( match, ITEM, itemname, sizeof( itemname ) );
			//
	...
	if ( match->type == MSG_HELP ) {
		...
		if ( !bs->teamgoal_time ) {
			bs->teamgoal_time = sys->AAS_Time() + TEAM_HELP_TIME;
		}
	} else {
		bs->ltgtype = LTG_TEAMACCOMPANY;
		if ( !bs->teamgoal_time ) {
			bs->teamgoal_time = sys->AAS_Time() + TEAM_ACCOMPANY_TIME;
		}
...
void BotMatch_DefendKeyArea( bot_state_t *bs, bot_match_t *match ) {
	...
	//get the match variable
	sys->BotMatchVariable( match, KEYAREA, itemname, sizeof( itemname ) );
	//
	if ( !BotGetMessageTeamGoal( bs, itemname, &bs->teamgoal ) ) {
		//BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
		//sys->BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
...
void BotMatch_GetItem( bot_state_t *bs, bot_match_t *match ) {
	...
	//set the team goal time
	bs->teamgoal_time = sys->AAS_Time() + TEAM_GETITEM_TIME;
#ifdef DEBUG
	BotPrintTeamGoal( bs );
#endif //DEBUG
}
...
void BotMatch_Camp( bot_state_t *bs, bot_match_t *match ) {
	...
	//set the team goal time
	if ( !bs->teamgoal_time ) {
		bs->teamgoal_time = sys->AAS_Time() + TEAM_CAMP_TIME;
	}
...
void BotMatch_Patrol( bot_state_t *bs, bot_match_t *match ) {
	...
	if ( !bs->teamgoal_time ) {
		bs->teamgoal_time = sys->AAS_Time() + TEAM_PATROL_TIME;
	}
	//
#ifdef DEBUG
	BotPrintTeamGoal( bs );
...
void BotMatch_ReturnFlag( bot_state_t *bs, bot_match_t *match ) {
	...
	//set the team goal time
	bs->teamgoal_time = sys->AAS_Time() + CTF_RETURNFLAG_TIME;
	bs->rushbaseaway_time = 0;
#ifdef DEBUG
	BotPrintTeamGoal( bs );
#endif //DEBUG
...
void BotMatch_JoinSubteam( bot_state_t *bs, bot_match_t *match ) {
	...
	sys->BotMatchVariable( match, TEAMNAME, teammate, MAX_MESSAGE_SIZE );
...
void BotMatch_CheckPoint( bot_state_t *bs, bot_match_t *match ) {
	...
	//
	sys->BotMatchVariable( match, POSITION, buf, MAX_MESSAGE_SIZE );
	...
	//
	sys->BotMatchVariable( match, NAME, buf, MAX_MESSAGE_SIZE );
...
void BotMatch_FormationSpace( bot_state_t *bs, bot_match_t *match ) {
	...
	//
	sys->BotMatchVariable( match, NUMBER, buf, MAX_MESSAGE_SIZE );
...
void BotMatch_StartTeamLeaderShip( bot_state_t *bs, bot_match_t *match ) {
	...
	if ( match->subtype & ST_I ) {
		...
		sys->BotMatchVariable( match, NETNAME, teammate, sizeof( teammate ) );
...
void BotMatch_WhoIsTeamLeader( bot_state_t *bs, bot_match_t *match ) {
	...
	if ( !Q_stricmp( netname, bs->teamleader ) ) {
		sys->EA_SayTeam( bs->client, "I'm the team leader\n" );
	}
}
...
void BotMatch_WhatAreYouDoing( bot_state_t *bs, bot_match_t *match ) {
	...
	case LTG_DEFENDKEYAREA:
	{
		sys->BotGoalName( bs->teamgoal.number, goalname, sizeof( goalname ) );
...
float BotNearestVisibleItem( bot_state_t *bs, char *itemname, bot_goal_t *goal ) {
	...
	i = -1;
	do {
		i = sys->BotGetLevelItemGoal( i, itemname, &tmpgoal );
		sys->BotGoalName( tmpgoal.number, name, sizeof( name ) );
...
void BotMatch_WhereAreYou( bot_state_t *bs, bot_match_t *match ) {
	...
	if ( bestitem != -1 ) {
		if ( gametype == GT_CTF ) {
			redflagtt = sys->AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, ctf_redflag.areanum, TFL_DEFAULT );
			blueflagtt = sys->AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, ctf_blueflag.areanum, TFL_DEFAULT );
			redtobluett = sys->AAS_AreaTravelTimeToGoalArea( ctf_redflag.areanum, ctf_redflag.origin, ctf_blueflag.areanum, TFL_DEFAULT );
...
void BotMatch_LeadTheWay( bot_state_t *bs, bot_match_t *match ) {
	...
	bs->lead_time = sys->AAS_Time() + TEAM_LEAD_TIME;
	bs->leadvisible_time = 0;
	bs->leadmessage_time = -( sys->AAS_Time() + 2 * random() );
}
...
void BotMatch_Kill( bot_state_t *bs, bot_match_t *match ) {
	...
	sys->BotMatchVariable( match, ENEMY, enemy, sizeof( enemy ) );
	//
	...
	bs->ltgtype = LTG_KILL;
	//set the team goal time
	bs->teamgoal_time = sys->AAS_Time() + TEAM_KILL_SOMEONE;
#ifdef DEBUG
	BotPrintTeamGoal( bs );
#endif //DEBUG
}
...
void BotMatch_CTF( bot_state_t *bs, bot_match_t *match ) {
	...
	sys->BotMatchVariable( match, FLAG, flag, sizeof( flag ) );
...
int BotMatchMessage( bot_state_t *bs, char *message ) {
	...
	if ( !sys->BotFindMatch( message, &match, MTCONTEXT_ENTERGAME
	...
	{
		sys->EA_SayTeam( bs->client, "the part of my brain to create formations has been damaged" );
		break;
	}
