...
void CCamWnd::OnPaint() 
{
  ...
  idGraphicsDeviceContextHelper context(dc.m_hDC, g_qeglobals.d_hglrcBase);
...
int CCamWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  if ((g_qeglobals.d_hglrcBase = (HGLRC)wglCreateContext(g_qeglobals.d_hdcBase)) == 0)
...
void CCamWnd::SetProjectionMatrix() {
	...
	float yfov = 2 * atan((float)m_Camera.height / m_Camera.width) * M_RAD2DEG;
#if 0
	...
	xmax = zNear * tan(xfov * M_PI / 360.0f);
	xmin = -xmax;
