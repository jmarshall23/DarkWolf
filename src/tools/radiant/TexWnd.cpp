...
void Texture_SetMode(int iMenu)
{
  ...
  idGraphicsDeviceContextHelper context(s_hdcTexture, s_hglrcTexture);
...
qtexture_t *Texture_LoadTexture (miptex_t *qtex)
{
  ...
  idGraphicsDeviceContextHelper context(g_qeglobals.d_hdcBase, g_qeglobals.d_hglrcBase);
      ...
      // we need to try several formats here, or would it be better if we are given a complete name
        ...
        if (pPixels == NULL)
        {
          ...
          // with GL plugins that have their own window, the GL context may be the plugin's, in which case loading textures will bug
	        ...
	        HGLRC currentHGLRC = (HGLRC)wglGetCurrentContext();
    ...
    LPARAM  lParam)
{
    ...
    s_hdcTexture = GetDC(hWnd);
		...
		if ( ( s_hglrcTexture = (HGLRC)wglCreateContext( s_hdcTexture ) ) == 0 )
