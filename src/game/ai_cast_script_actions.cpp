...
qboolean AICast_ScriptAction_MissionFailed( cast_state_t *cs, char *params ) {
	...
	sys->SendServerCommand( -1, "mu_play sound/music/l_failed_1.wav 0\n" );
	// clear queue so it'll be quiet after failed stinger
	sys->SetConfigstring( CS_MUSIC_QUEUE, "" );

	// fade all sound out
	sys->SendServerCommand( -1, va( "snd_fade 0 %d", time * 1000 ) );
	...
	sys->SetConfigstring( CS_SCREENFADE, va( "1 %i %i", level.time + 250, time * 1000 ) );
//	reloading = RELOAD_FAILED;
	sys->Cvar_Set( "g_reloading", va( "%d", RELOAD_FAILED ) );
...
// if you've already got it, just return.  don't need to set 'yougotmail'
	...
	sys->Cvar_Register( &cvar, va( "g_objective%i", lvl ), "1", CVAR_ROM );
	// set it to make sure
...
qboolean AICast_ScriptAction_Mount( cast_state_t *cs, char *params ) {
	...
	if ( dist > 40 ) {
		// walk towards it
		sys->EA_Move( cs->entityNum, vec, 80 );
		return qfalse;
	}
...
//	if(reloading)
	...
	if ( !endgame ) {
		...
		for ( i = 0; i < level.numObjectives; i++ ) {
			if ( !( player->missionObjectives & ( 1 << i ) ) ) {
				sys->SendServerCommand( -1, "cp objectivesnotcomplete" );
				return qtrue;
			}
		}
	...
	sys->SendServerCommand( -1, va( "snd_fade 0 %d", 1000 + exitTime ) ); //----(SA)	added

	// load the next map, after a delay
	level.reloadDelayTime = level.time + 1000 + exitTime;
	sys->Cvar_Set( "g_reloading", va( "%d", RELOAD_NEXTMAP_WAITING ) );

	if ( endgame ) {
		sys->Cvar_Set( "g_reloading", va( "%d", RELOAD_ENDGAME ) );
		return qtrue;
	}
...
//	level.numSecretsFound++;
	...
	sys->SendServerCommand( -1, "cp secretarea" );
	G_SendMissionStats();
	return qtrue;
}
...
qboolean ScriptStartCam( cast_state_t *cs, char *params, qboolean black ) {
	...
	sys->SendServerCommand( cs->entityNum, va( "startCam %s %d", token, (int)black ) );
...
qboolean AICast_ScriptAction_StopCamBlack( cast_state_t *cs, char *params ) {
	sys->SendServerCommand( cs->entityNum, "stopCamblack" );
	return qtrue;
}
...
qboolean AICast_ScriptAction_EntityScriptName( cast_state_t *cs, char *params ) {
	sys->Cvar_Set( "g_scriptName", params );
	return qtrue;
}
...
qboolean AICast_ScriptAction_AIScriptName( cast_state_t *cs, char *params ) {
	sys->Cvar_Set( "ai_scriptName", params );
	return qtrue;
}
...
qboolean AICast_ScriptAction_Cvar( cast_state_t *cs, char *params ) {
	...
	sys->Cvar_Register( &cvar, cvarName, token, CVAR_ROM );
	// set it to make sure
	sys->Cvar_Set( cvarName, token );
	return qtrue;
}
...
qboolean AICast_ScriptAction_MusicStart( cast_state_t *cs, char *params ) {
	...
	sys->SendServerCommand( cs->entityNum, va( "mu_start %s %d", cvarName, fadeupTime ) );
...
qboolean AICast_ScriptAction_MusicStop( cast_state_t *cs, char *params ) {
	...
	sys->SendServerCommand( cs->entityNum, va( "mu_stop %i", fadeoutTime ) );
