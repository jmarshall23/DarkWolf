...
// jpw
	...
	if ( g_gametype.integer == GT_SINGLE_PLAYER ) {
		...
		sys->SendServerCommand( -1, "cp missionfail0" );
	}
...
qboolean IsHeadShot( gentity_t *targ, gentity_t *attacker, vec3_t dir, vec3_t point, int mod ) {
	...
	if ( head_shot_weapon ) {
		...
		sys->Trace( &tr, start, NULL, NULL, end, targ->s.number, MASK_SHOT );
...
// jpw
	...
	numListedEntities = sys->EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES );
