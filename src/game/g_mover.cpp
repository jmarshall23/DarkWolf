...
gentity_t   *G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		sys->TraceCapsule( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number, mask );
	} else if ( ent->s.eType == ET_MISSILE ) {
		sys->Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->r.ownerNum, mask );
	} else {
		sys->Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask );
	}
...
void G_TestEntityDropToFloor( gentity_t *ent, float maxdrop ) {
	...
	endpos[2] -= maxdrop;
	if ( ent->client ) {
		sys->TraceCapsule( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, endpos, ent->s.number, mask );
	} else {
		sys->Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, endpos, ent->s.number, mask );
	}
...
void G_TestEntityMoveTowardsPos( gentity_t *ent, vec3_t pos ) {
	...
	if ( ent->client ) {
		sys->TraceCapsule( &tr, ent->client->ps.origin, ent->r.mins, ent->r.maxs, pos, ent->s.number, mask );
	} else {
		sys->Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, pos, ent->s.number, mask );
	}
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	sys->UnlinkEntity( pusher );

	listedEntities = sys->EntitiesInBox( totalMins, totalMaxs, entityList, MAX_GENTITIES );

	// move the pusher to it's final position
	VectorAdd( pusher->r.currentOrigin, move, pusher->r.currentOrigin );
	VectorAdd( pusher->r.currentAngles, amove, pusher->r.currentAngles );
	sys->LinkEntity( pusher );
...
void ReturnToPos1Rotate( gentity_t *ent ) {
	...
	if ( player ) {
		inPVS = sys->InPVS( player->r.currentOrigin, ent->r.currentOrigin );
	}
...
void SP_func_door( gentity_t *ent ) {
	...
	sys->SetBrushModel( ent, ent->model );
...
void SP_func_train( gentity_t *self ) {
	...
	sys->SetBrushModel( self, self->model );
	InitMover( self );
...
/*
		...
		if (bat->damage < level.time) {
			sys->Trace( &tr, bat->r.currentOrigin, NULL, NULL, bat->r.currentOrigin, bat->s.number, CONTENTS_BODY );
...
void use_target_effect( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	sys->SetConfigstring( CS_TARGETEFFECT, self->dl_shader );   //----(SA)	allow shader to be set from entity
