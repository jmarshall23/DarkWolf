...
void snow_think( gentity_t *ent ) {
	...
	sys->Trace( &tr, ent->s.origin, NULL, NULL, dest, ent->s.number, MASK_SHOT );
...
void Fire_Lead( gentity_t *ent, gentity_t *activator, float spread, int damage ) {
	...
	sys->Trace( &tr, lead_muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT );
