...
//----(SA)	end
	...
	if ( item->icon && loadingItemIconCount < MAX_LOADING_ITEM_ICONS ) {
		loadingItemIcons[loadingItemIconCount++] = sys->R_RegisterShaderNoMip( item->icon );
	}
...
//			loadingPlayerIcons[loadingPlayerIconCount++] = sys->R_RegisterShaderNoMip( iconName );
//		}
	}
	...
	if ( cgs.gametype == GT_SINGLE_PLAYER ) {
		sys->S_RegisterSound( va( "sound/player/announce/%s.wav", personality ) );
	}
...
//	qhandle_t	detail;
	...
	if ( s && s[0] != 0 ) {  // there is often no 's'
		levelshot = sys->R_RegisterShaderNoMip( va( "levelshots/%s.tga", s ) );
	}
...
//	CG_DrawPic( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, levelshot );
	...
	//detail = sys->R_RegisterShader( "levelShotDetail" );
	//sys->R_DrawStretchPic( 0, 0, cgs.glconfig.vidWidth, cgs.glconfig.vidHeight, 0, 0, 2.5, 2, detail );
